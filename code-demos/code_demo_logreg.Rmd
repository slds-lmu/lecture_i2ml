...
```
For complete separation the empirical risk is monotonously decreasing,
...
The Newton method follows this algorithm:
...
$$\frac{\partial R_{emp}(f)}{\partial \theta}\bigg|_{\theta=t}  = \sum_{i=1}^n \frac{-y^{(i)}}{1+\exp(y^{(i)}{\theta^{[t]}}^T x^{(i)})}x^{(i)},$$
$$\frac{\partial^2 R_{emp}(f)}{\partial \theta^2}\bigg|_{\theta=t}  =  \sum_{i=1}^n \frac{\exp(y^{(i)}{\theta^{[t]}}^T x^{(i)})}{(1+\exp(y^{(i)}{\theta^{[t]}}^T x^{(i)}))^2}x^{(i)}{x^{(i)}}^T.$$
2. update the estimate for $\theta$ using this formula:
$$
\theta^{[t+1]} = \theta^{[t]} - \lambda \bigg[\frac{\partial^2 R_{emp}(f)}{\partial \theta^2}\bigg|_{\theta=t}\bigg]^{-1} \frac{\partial R_{emp}(f)}{\partial \theta}\bigg|_{\theta = \theta^{[t]}}
$$
...
now we use a "weighted" gradient.
...
ggplot() + 
  ...
  geom_vline(xintercept = 6.5, color="yellow") + 
  geom_abline(slope=slope, intercept = intercept, color="turquoise") + 
  geom_text(aes(label='x^{~"[1]" }',x=x1, y=-1.5),parse = TRUE) +
  geom_text(aes(label='x^{~"[0]" }',x=x0, y=-1.5),parse = TRUE) +
  geom_text(aes(label='"f\'"(x^{~"[0]" })',x=x0+0.3, y=f(x0)),parse = TRUE,
            color="turquoise") + 
  geom_text(aes(label='f(x)',x=4, y=20),parse = TRUE,
            color="purple")
```

If our scalar function $f$ of interest has a root and we are *near enough* to it, we can 
find it by iteratively using following procedure:
$$
x^{[t]} = x^{[t-1]} - f'(x^{[t-1]})\cdot f(x^{[t-1]}).
$$
Applying the multidimensional version of this to our gradient function results in the Newton-Raphson method.
This makes sense since a necessary condition for a minimum is, that the gradient is zero, i.e., for the empirical risk that
$$
