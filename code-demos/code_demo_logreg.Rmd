...
ggplot() + 
  ...
  geom_curve(data=data.frame(x=x1+0.3, y=0, x_to=x1+0.2, y_to=slope*(x1+0.2)+intercept), aes(x=x,y=y,xend=x_to,yend=y_to),angle = 90,curvature = 0.3, color="black") + 
  geom_text(aes(label='x^{~"[t+1]" }',x=x1, y=-1.5),parse = TRUE) +
  geom_text(aes(label='x^{~"[t]" }',x=x0, y=-1.5),parse = TRUE) +
  geom_text(aes(label='tan^-1 ~ ( ~ "f\'"(x^{~"[t]" }))  ',x=x1+1, y=3),parse = TRUE,
            color="black") + 
  geom_text(aes(label='f(x)',x=4, y=20),parse = TRUE,
            color="purple") +
   geom_text(aes(label='f(x^{~"[t]" })',x=x0+0.3, y=f(x0)),parse = TRUE,
            color="black")
```

The idea for finding a root candidate consists in following the negative gradient from our starting point.
By applying simple geometry one derives that
$$
