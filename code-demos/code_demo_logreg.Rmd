...
classify_logreg <- function(X, theta, Xtheta= X%*% theta){
  pred <- as.character(2*as.integer(logistic(X, theta,  Xtheta=Xtheta) > 0.5)-1)
  list(prediction = pred, levels = c("1","-1"))
}
```
...
X <- cbind(rep(1,n),matrix(runif(n*p, -5, 5), nrow = n))
...
Y <- as.integer(classify_logreg(X, theta)$pred)
```
...
plot_2D_logreg <- function(theta, X1="X1",X2="X2",
                      ...
                      to_classify_data = data.frame("X1"=X[,2],
                                                   "X2"=X[,3]),
  ...
  plot_2D_classify(to_classify_labels = to_classify_labels,
                   ...
                   classify_method = function(to_classify_data) 
                       classify_logreg(
                               X = cbind(rep(1,nrow(to_classify_data)),
                                         as.matrix(to_classify_data)),
                              theta = theta),
                   X1, X2,
...
plot2D_emp_risk(emp_risk = function(theta) (risk_bernoulli(Y,X,c(2,theta))),
                max_theta = c(100,100), min_theta = c(-100,-100), 
                log_emp_risk = TRUE)
```

```{r logreg-example_grad_desc}
gradient_descent_opt_stepsize(Y,X,theta,
                     risk_bernoulli,
                     gradient_bernoulli,
