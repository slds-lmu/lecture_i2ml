...
bs_plot <- function(poly_deg = 1,
  ...
  ggplot(plot_data, aes(x = x, color = variable)) +
    ...
    ylab("y") + theme(legend.position = "none") + scale_color_brewer(palette =
                                                                       "Set3") + labs(title = paste("Piecewise polynomial degree:", as.character(poly_deg)))
}
...
# estimate a linear model for transformed features and exclude the intercept 
# term  as it has already been included in the b-splines basis functions
...
ggplot_list <- lapply(num_bfuns, function(x)
    plot_bs_fit(data = BostonHousing2, x = "rm", y = "medv", poly_deg = poly_deg,
