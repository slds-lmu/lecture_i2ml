...
```{r randforests-mnist_prox_3d}

proxMat <- mod$learner.model$proximity

# apply mds on the proximity matrix
mds <- as.data.frame(cmdscale(proxMat, k=3))
mds$class <- train_data$X1
mds$id <- 1:nrow(mds)


# plot the result
ggplot(data = mds, aes(x = V1, y = V2, colour = class)) +
  geom_point(alpha = 0.5) +
  geom_text_repel(
    data = subset(mds, id %in% c(4,132,48, 137, 144, 80,
                                 130, 89, 86, 146, 117)),
...
ggplot(data = mds, aes(x = V2, y = V3, colour = class)) +
  geom_point(alpha = 0.5) +
  geom_text_repel(
    data = subset(mds, id %in% c(4,132,48, 137, 144, 80,
                                 130, 89, 86, 146, 117)),
  ...
  labs(
    x = "2nd dimension", y = "3rd dimension", title =
      "Multidimensional Scaling based on proximity"
  )

ggplot(data = mds, aes(x = V1, y = V3, colour = class)) +
  geom_point(alpha = 0.5) +
  geom_text_repel(
    data = subset(mds, id %in% c(4,132,48, 137, 144, 80,
                                 130, 89, 86, 146, 117)),
  ...
  labs(
    x = "1st dimension", y = "3rd dimension", title =
      "Multidimensional Scaling based on proximity"
  )

```

```{r randforests-mnist_3_digits}
mnist_splits <- sapply(0:9, function(x)  mnist_raw[ mnist_raw[, 1] == x, ], 
                       simplify = F)
set.seed(123)
mnist_train <- sapply(mnist_splits, function(x) 
  x[sample(nrow(x), train_size / 2, replace = FALSE),], simplify = F)


train_data <-  as.data.frame(do.call(rbind,mnist_train))
train_data$X1 <- as.factor(train_data$X1)
