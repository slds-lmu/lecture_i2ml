...
plot_mnist_raster <- function(pixels) {
  m <- matrix(as.numeric(pixels), nrow = 28, byrow = F)
  image(rotate(t(m)), axes = FALSE, col = gray.colors(256))
}
...
p <- ggplot(data = mds, aes(x = V1, y = V2, colour = class)) +
  ...
  geom_text_repel(
    data = subset(mds, id == 4 | id == 132 | id == 103),
  ...
  labs(
    x = "1st dimension", y = "2nd dimension", title =
      "Multidimensional Scaling of MNIST data based on proximity"
  )
p

plot_mnist_raster(train_data[4, 2:ncol(train_data)])
plot_mnist_raster(train_data[132, 2:ncol(train_data)])

plot_mnist_raster(train_data[103, 2:ncol(train_data)])
```
