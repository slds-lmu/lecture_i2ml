...
plot_2D_knn <- function(train_labels, train_data, k, X1, X2,
  ...
  # assign data to be classified based on k-NN rule & check these "predictions"
  to_check_result <- get_knn(
    train_labels = train_labels,
    train_data = train_data[, c(X1, X2)],
...
```
For $k=1$ we can only ever get an if there exist observations with identical feature values but different labels in our training data set. So why should we use a $k \neq 1$?
