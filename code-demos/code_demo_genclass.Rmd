...
plot_2D_likelihood <- function(likelihoods, data, X1, X2, target, lengthX1 = 100, 
  ...
  # lik <- sapply(names(likelihoods), 
  #       function(level) exp(rowSums(log(liks[[level]]))))
...
plot_2D_likelihood <- function(likelihoods, data, X1, X2, target, lengthX1 = 100, 
  ...
  grid_data <- expand.grid(gridX1, gridX2)  
  features <-  c(X1, X2)
  levels <- names(likelihoods)
  names(grid_data) <- features
  
  liks <- sapply(names(likelihoods), function(level) { 
    sapply(features, function(feature){
       likelihood <- likelihoods[[level]][[feature]]
       predict_likelihood(likelihood, grid_data[,feature])
    } )
  }, simplify = FALSE)
...
naivebayes <- function(X, priors, likelihoods, normalize=FALSE){

  features <- colnames(X)
  
  lik <- sapply(names(priors), function(level) { 
    sapply(features, function(feature){
       likelihood <- likelihoods[[level]][[feature]]
       predict_likelihood(likelihood,X[,feature])
    } )
  }, simplify = FALSE)
  
  joint_density <- sapply(names(priors), 
        function(level) exp(log(priors[[level]]) + rowSums(log(lik[[level]]))))
...
likelihoods_kde <- get_likelihoods(iris_train,target,sapply(colnames(iris_train[,!(names(iris_train) %in% target)]), function(feature)  list(args = list(), type = "kde"), 
                           simplify = FALSE,
                           USE.NAMES = TRUE))
