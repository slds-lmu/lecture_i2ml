...
get_priors <- function(df,target){
  counts <- count(df[,target_labels])
  # prop.table(table(df[,target_labels]))?
...
gaussian_est <- function(data) {#gaussian_dens?
  m_data <- mean(data, na.rm = TRUE)
  sd_data <- sd(data, na.rm = TRUE)
  function(x) {
    dnorm(x = x, mean = m_data, sd = sd_data)
  }
}
  

get_likelihoods <- function(df,target, density__type = c("gausssian", "kde"), 
                            bw = NA, kernel = "gaussian") {
  # function generator vermeiden. 
  X <- df[,!(names(df) %in%target)]
  features <- colnames(X)
  target_levels <- levels(df[,target_labels])
  # default to gaussian density estimates:
  if (is.null(density_funs)) {
    density_funs <- sapply(features, 
                           function(feature) gaussian_est)
  ...
  likelihoods <- lapply(
    ...
    function(level) {
      sapply(features, function(feature) {
        density_fun_generator <- density_funs[[feature]]
        feature_data <- X[df[[target_labels]] == level, feature]
        force(density_fun_generator(feature_data))
      })
    })
...
naivebayes <- function(X, priors, likelihoods){
  res <- replicate(length(priors), 
                   matrix(data=NA_real_,nrow=nrow(X),ncol=length(priors)))
  features <- colnames(X)
  # for level
  #   for feature
...
priors <- get_priors(iris_train,target_labels)
...
likelihoods_kde <- get_likelihoods(iris_train,target,
    sapply(colnames(iris_train[,!(names(iris_train) %in%target)]), 
