...
stairs <- function(n, steps){
  ...
  for(i in 1:(steps-1)){
    for(j in 1:i){
      xs[xs$block == block,1:2] <- t(t(xs[xs$block == block,1:2]) + c(i/steps, 
                                                    j/steps))
...
ggplot(sim_stairs, aes(x=V1, y=V2)) + geom_point(aes(color=classes), alpha=0.5)
...
for(k in k_candidates){
  ...
  if(is.null(mmces)){
    mmces <- knn_cv(sim_stairs, "classes", 10, k)$storage   
  }else{
    mmces <- cbind(mmces,  knn_cv(sim_stairs, "classes", 10, k)$storage)
  }
...
So from this we would choose $k = 28$, since it has the smallest MSE. But how can we estimate the 
...
task <- makeClassifTask(data = sim_stairs, target = "classes")
learner <- makeLearner("classif.kknn", k = 28)
model <- train(learner = learner, task = task)
performance(predict(model, newdata = unseen_data), measures = mmce)[[1]]
...
```{r nested-mlr_impl}
ps <- makeParamSet(
  makeDiscreteParam("k", values = k_candidates)
)
