...
Now we want to implement a simple regression tree (`rt`) for numerical features:

```{r rt-impl}
# Function which generates the *r*egression *t*ree which consists of nodes.
# Every node has 2 subnodes or no subnode:
#  - true_node: the split citerin was TRUE
#  - false_node: the split citerion was FALSE
# A node without subnodes is a leaf. They represent the end of a tree branch.
rt <- function(df, target, cur_depth = 0, max_depth = 30, min_split = 20) {
  ...
  list(
    ...
    threshold = min_loss_split$threshold,
...
predict_tree <- function(tree, df) {
  ...
  for (i in 1:nrow(df)) {
    cur_node <- tree
    while (!cur_node$is_leaf) { # follow the splits until we reach a leaf
