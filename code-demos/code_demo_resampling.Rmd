...
results <- lapply(X = c(2:10), FUN = function(times)
  data.frame(times = as.character(times), 
             sapply(X = 1:times, FUN = function(x) knn_cv(data = spiral, target = "classes", folds = 5, k = 4)$gen_error)))

plot_data <- do.call(rbind, results)

colnames(plot_data) <- c("times", "storage")

ggplot(plot_data, aes(x=times, y=storage)) + geom_boxplot() + xlab("times 5-CV") +
  ylab("test error")
```
