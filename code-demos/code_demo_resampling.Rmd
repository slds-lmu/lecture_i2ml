...
knn_cv <- function(data, target, folds, k) {
  ...
  index_mat <- matrix(data = indices, byrow = TRUE, nrow = folds)

  for (i in 1:folds) {

    # data
    test_data <- data[index_mat[i, ], ]
    train_data <- data[-index_mat[i, ], ]
...
result <- knn_cv(data = spiral, target = "classes", folds = 10, k = 3)
result$storage
result$gen_error
```
