...
\begin{itemize}
  ...
  \item The actual classification rule is usually defined as: $h(x) = \displaystyle \argmax_{\ell \in \{1, \dots ,g\}} f_\ell(x)$
  \item This classification rule also defines the class boundaries in the feature space $\mathcal X.$
\end{itemize}
