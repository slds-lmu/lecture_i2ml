...
ggplot(df2, aes(x = x1, y = x2, shape = class, color = class)) + 
  geom_point(size = 3) + geom_vline(xintercept = 4, linetype = "longdash") + 
...
\lz
  ...
  \begin{itemize}
    \item Massive amount of labeled data (expensive) to train reliable model
...
\begin{itemize}
  ...
  \item The machine learning problem remains: How do we generalize from cases that we haven't seen before?
    ...
    \end{itemize}
  \item Even with all the data in the world, your chances of finding the exact same case are almost zero. We need induction.
\end{itemize}
\end{vbframe}
