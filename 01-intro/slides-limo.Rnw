...
<<echo=FALSE, out.width="0.8\\textwidth", fig.width=4.5, fig.height=3>>=
ggplot() +
  ...
  ylim(c(0, 3.5)) + xlim(c(-0.5, 4.3)) + theme_bw()
@
...
<<echo=FALSE, out.height="0.75\\textheight", out.width="0.75\\textwidth", fig.width=5, fig.height=5>>=
frame = 5
source("figure_man/sse_lm_viz.R")
@
...
\begin{align*}
\hat{\thetab} &= \argmin_{\thetab} \risket = \sumin \left(\yi - \thetab^T \xi\right)^2\\
...
\[
\frac{\partial}{\partial\thetab} \risket = 0 \qquad \Rightarrow\ \qquad \hat{\thetab} = \left(\Xmat^T \Xmat\right)^{-1}\Xmat^T\yv
\]
...
plot(x.plot,
     ...
     .h(x.plot) + .1 * rnorm(length(x.plot)), pch = 19, col = rgb(0,0,0,.5),
