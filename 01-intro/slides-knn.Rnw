...
The Gower distance \(d_{ij}\) is a weighted mean of \(d_{ij}^{(k)}\):
...
\begin{itemize}
  \item \(w_k\)\texttt{= weigths[k]}
  
  \item \(\delta_{ij}^{(k)}\) is 0 or 1. It becomes 0
  when the variable \texttt{x[,k]} is \textbf{\textit{missing}} in either or both rows
  (i and j), or when the variable is asymmetric binary (where one of the states is more 
  important, e. g. a medical test) and both
  values are zero. Otherwise it is 1.
  
  \item \(d_{ij}^{(k)}\), the k-th variable contribution to the
  total distance, is a distance between \texttt{x[i,k]} and \texttt{x[j,k]}.
...
$d_{1,2} = \frac{1 \cdot 1 \cdot 1 + 1 \cdot 1 \cdot \frac{\left| 2340 - 2100 \right|}
{\left| 2680 - 2100 \right|}}{1 + 1} = \frac{1 + \frac{240}{580}}{2} = \frac{1 + 0.414}{2} = 0.707
$

\vfill

$d_{1,3} = \frac{1 \cdot 0 \cdot 1 + 1 \cdot 1 \cdot \frac{\left| 2340 - 2680 \right|}
{\left| 2680 - 2100 \right|}}{0 + 1} = \frac{0 + \frac{340}{580}}{1} = \frac{0 + 0.586}{1} = 0.586
$

\vfill

$d_{2,3} = \frac{1 \cdot 0 \cdot 1 + 1 \cdot 1 \cdot \frac{\left| 2100 - 2680 \right|}
