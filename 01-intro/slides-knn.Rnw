...
\textbf{How to calculate distances?}
  \begin{itemize}
    ...
    \item Imagine two data points $x = (x_1, ..., x_q)$ and $\tilde{x} = (\tilde{x}_1, ..., \tilde{x}_q)$ with $q$ features $\in \R$
    \item The Euclidean distance:
    \begin{equation*}
      d_{Euclidean}\left(x, \tilde{x}\right) = \sqrt{\sum_{j=1}^q(x_j- \tilde{x}_j)^2}
    \end{equation*}
...
\end{itemize}
    ...
    \begin{itemize}
      \item Manhattan distance based on the $L_1$-norm:
        \begin{equation*}
          d_{manhattan}(x, \tilde{x}) = \sum_{j=1}^q(|x_j - \tilde{x}_j|)
        \end{equation*}
...
\textbf{Categorical variables and mixed space:}

The Gower distance \(d_{gower}(x,\tilde{x})\) is a weighted mean of \(d_{gower}(x_j,\tilde{x}_j)\):

$$d_{gower}(x,\tilde{x}) = \frac{\sum_{j=1}^q \delta_{x_j,\tilde{x}_j} \cdot d_{gower}(x_j,\tilde{x}_j)}{
\sum_{j=1}^q \delta_{x_j,\tilde{x}_j}}.
$$

\begin{itemize}
  
  \item \(\delta_{x_j,\tilde{x}_j}\) is 0 or 1. It becomes 0
  when the $j$-th variable is \textbf{\textit{missing}} in either or both data points ($x$ or $\tilde{x}$), 
  or when the variable is asymmetric binary (where one of the states is more 
  important, e.~g. a medical test; has to be defined by modeller) and both
  values are zero. Otherwise it is 1.
  
  \item \(d_{gower}(x_j,\tilde{x}_j)\), the $j$-th variable contribution to the
  total distance, is a distance between the $j$-th variables of $x$ and $\tilde{x}$.
  For nominal or binary variables the total
...
$d_{gower}(1,2) = \frac{ 1 \cdot 1 + 1 \cdot \frac{\left| 2340 - 2100 \right|}
{\left| 2680 - 2100 \right|}}{1 + 1} = \frac{1 + \frac{240}{580}}{2} = \frac{1 + 0.414}{2} = 0.707
$

\vfill

$d_{gower}(1,3) = \frac{ 0 \cdot 1 + 1 \cdot \frac{\left| 2340 - 2680 \right|}
{\left| 2680 - 2100 \right|}}{0 + 1} = \frac{0 + \frac{340}{580}}{1} = \frac{0 + 0.586}{1} = 0.586
$

\vfill

$d_{gower}(2,3) = \frac{0 \cdot 1 + 1 \cdot \frac{\left| 2100 - 2680 \right|}
...
Weights can be used to address to problems in distance calculation:

\begin{itemize}
  \item \textbf{Standardisation:} Two features may have values with a different scale. Many distance formulae (not Gower) would place a higher importance on a feature with higher values leading to an imbalance. Assigning a higher weight for the lower valued feature can combat this effect.
...
\begin{columns}[T]
  ...
  \begin{column}{0.8\textwidth}
     $d^{weighted}_{Euclidean}\left(x, \tilde{x}\right) = \sqrt{\sum_{j=1}^qw_{j}(x_j- \tilde{x}_j)^2}$
  \end{column}
\end{columns}
