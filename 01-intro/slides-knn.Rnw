...
Categorical variables and mixed space:

Package \texttt{cluster} function \texttt{daisy} allows option \texttt{metric = "gower"}.
...
The Gower distance \(d_{ij}\) is a weighted mean of \(d_{ij}^{(k)}\):
      ...
      when the variable \texttt{x[,k]} is missing in either or both rows
