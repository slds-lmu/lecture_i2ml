...
\textbf{How to calculate distances?}
  \begin{itemize}
    \item Most popular distance measure for metric variables: \textbf{euclidian distance}
    \item Imagine two data points $a = (a_1, ..., a_q)^T$ and $b = (b_1, ..., b_q)^T$ with $q$ metric variables
    \item The euclidean distance between the points follows the formula:
    \begin{equation*}
      $d_{euclidean}(a, b) = \sqrt{\sum_{i=1}^q(a_i - b_i)^2}$
    \end{equation*}
...
\framebreak
    ...
    \begin{itemize}
      \item Manhattan distance based on the $L_1$-norm:
        \begin{equation*}
          d_{manhattan}(a, b) = \sum_{i=1}^q(|a_i - b_i|)
        \end{equation*}
      \item Mahalanobis distance
    \end{itemize}
    \item Example:
    \begin{itemize}
      \item Three data points with two metric features each: $a = (1, 3)^T,
      b = (4, 5)^T$ and $c = (7, 8)^T$
      \item Which is the nearest neighbor for $b$ in terms of euclidean distance?
