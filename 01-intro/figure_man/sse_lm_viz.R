...
if (frame == 4) {
  ...
  frame_plot(x, y, coef3, colRed, c(2, 5.5), c(x = 6.5, y =  -0.5))
  p <- eval(sse_blank)
  points(trans3d(x = cbind(coef1, coef2, coef3)[1,], y = cbind(coef1, coef2, coef3)[2,], 
                 z = rep(min(sse_surf), 3),
                 pmat = p), col = c(colBlue(255), colOrange(255), colRed(255)), pch = 16, cex = 2)
  lines(trans3d(x = cbind(coef1, coef1)[1,], y = cbind(coef1, coef1)[2,], 
                 z = c(min(sse_surf), sse(coef1[1], coef1[2], x, y)),
                 pmat = p))
  lines(trans3d(x = cbind(coef2, coef2)[1,], y = cbind(coef2, coef2)[2,], 
                z = c(min(sse_surf), sse(coef2[1], coef2[2], x, y)),
                pmat = p))
  lines(trans3d(x = cbind(coef3, coef3)[1,], y = cbind(coef3, coef3)[2,], 
                z = c(min(sse_surf), sse(coef3[1], coef3[2], x, y)),
                pmat = p))
  points_3d = trans3d(x = cbind(coef1, coef2, coef3)[1,], y = cbind(coef1, coef2, coef3)[2,], 
                      z = c(sse(coef1[1], coef1[2], x, y), sse(coef2[1], coef2[2], x, y), 
...
if (frame == 7) {
  ...
  points_3d = 
    trans3d(x = cbind(coef1, coef2, coef3, coef(m))[1,], 
            ...
            z = c(sse(coef1[1], coef1[2], x, y), sse(coef2[1], coef2[2], x, y), 
                  sse(coef3[1], coef3[2], x, y), sse(coef(m)[1], coef(m)[2], x, y)),
            pmat = p)
  points(points_3d, col = c(colBlue(255), colOrange(255), colRed(255), colGreen(255)), 
         pch = 16, cex = 2)
  points(points_3d, col = "black", pch = 1, cex = 2, lwd = 2)
}  
