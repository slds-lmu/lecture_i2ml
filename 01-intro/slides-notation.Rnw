...
\item $\Yspace$ : target space, e. g. $\Yspace = \R$, $\Yspace = \lbrace 0, 1 \rbrace$, $\Yspace = \lbrace -1, 1 \rbrace$, $\Yspace = \gset$, $\Yspace = \lbrace \textrm{label}_1 \ldots \textrm{label}_g \rbrace$
\item $x$ : observation, $x = \xvec \in \Xspace$
\item $y$ : dependent variable (target, label, output), $y \in \Yspace$
