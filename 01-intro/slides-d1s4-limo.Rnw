...
\lecturechapter{4}{Linear regression models}
...
\begin{itemize}
  ...
  \llt \propto \sumin (\fxit - \yi)^2
  $$
  \item Which is equivalent to our loss minimization approach from the machine learning persepective!
  \item We can show this relation not only for a Gaussian distribution and L2 - loss but also for other loss-distribution-combinations (Laplacian - L2, Poisson - logistic loss, ...)
\end{itemize}
