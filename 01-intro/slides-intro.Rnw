...
\item \textbf{Classification}: Assigning an observation with features $x$ to one class of a finite set of classes $\mathcal{Y} = \{C_1,...,C_m\}, 2 \leq m < \infty$.
...
An \textbf{inducer} (learner, algorithm) takes a data set with features and outputs
...
\begin{frame}{Components of an Inducer}
...
\item
  \textbf{Optimization:} Defines how to search for the best model in the hypothesis space,   guided by the evaluation metric.
\item
