...
\item The learning algorithm (or inducer) $\mathcal{I}$ takes the input data, internally performs \textbf{empirical risk minimization}, and returns a fitted tree $\fxh = f(\xb, \thetah)$ of at most depth $4$ that minimizes the empirical risk.
\end{itemize}
...
\begin{itemize}
\item trained models often do not have a good generalization performance. 
...
\end{itemize}
\item $\implies$ try out different values for the tree depth. For each value of $\lambda$, we have to train the model \textbf{to completion}, and evaluate its performance on the test set. 
...
\begin{itemize}
\item Hyperparameters control the complexity of a model, i.e., how flexible the model is, as well as other structural properties of a model.
...
\item Hence, controlling the model's capacity, i.e., finding suitable hyperparameters,
...
\item A (resampling) procedure for estimating the predictive performance: The generalization error can be estimated by holdout (i.e., a single train-test-split), but also by more advanced techniques like cross-validation. 
\end{itemize}
...
\begin{itemize}
\item The splits and terminal node predictions of a tree learner 
...
\begin{itemize}
\item Numerical parameters (real valued / integers), e.g.:
\begin{itemize}
...
a "linear" model with polynomial features of a certain maximal degree $d$, then 
we must specify whether to include mixed polynomials like e.g. $x_k^{d-1} x_m$ or not.
\end{vbframe}

\begin{vbframe}{Tuning Pipelines}

\begin{itemize}
\item Many factors like optimization control settings, distance functions, scaling or centering of features, algorithmic variants in the fitting procedure can heavily influence model performance in non-trivial ways. It is extremely hard to guess the correct choices here.
