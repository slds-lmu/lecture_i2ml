...
\newcommand{\titlefigure}{figure/seq_1d_bo4.pdf}
...
\begin{document}
\lecturechapter{Hyperparameter Tuning - Advanced Tuning Techniques}
...
Since we use the sequentially updated surrogate model predictions of performance to propose new configurations, we are guided to \enquote{interesting} regions of $\bm{\Lambda}$ and avoid irrelevant evaluations:

\begin{center}
\begin{figure}
% see rsrc/mbo-example.R
% FIXME: Width was originally 0.6, but had to reduce to prevent overflow(?) resulting in empty new slide. Slightly downsizing this graphic seemed less intrusive than changing text or reducing text size. 0.56 is largest value I found that works, 0.57 is too large. // LBurk
\includegraphics[width=0.56\textwidth]{figure/mbo_random_tune.pdf}
