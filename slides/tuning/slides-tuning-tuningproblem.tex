...
training problem in which a learner $\ind$ minimizes the empirical risk on a training data set in order
...
\begin{eqnarray*}
    ...
    \GEhresa
\end{eqnarray*}
where $\lams$ denotes the theoretical optimum, and $\clam$ is a shorthand for the estimated generalization error when $\ind$, resampling splits $\JJ$, performance measure $\rho$ are fixed.
