...
\item The learner ("inducer") $\mathcal{I}$ takes the input data, internally performs \textbf{empirical risk minimization}, and returns a fitted tree model $\fxh = f(\xb, \thetah)$ of at most depth $\lambda = 4$ that minimizes the empirical risk.
\end{itemize}
