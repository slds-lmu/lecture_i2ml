...
\item The learning algorithm (or inducer) $\mathcal{I}$ takes the input data, internally performs \textbf{empirical risk minimization}, and returns a fitted tree $\fxh = f(\xb, \thetah)$ of at most depth $\lambda = 4$ that minimizes the empirical risk.
\end{itemize}
...
\begin{itemize}
\item Trained models often do not have a good generalization performance, however. 
...
\end{itemize}
\item[$\implies$] try out different values for the tree depth. For each maximal depth $\lambda$, we have to train the model \textbf{to completion} and evaluate its performance on the test set. 
...
a "linear" model with polynomials of the features up to a certain maximal degree $d$, then 
