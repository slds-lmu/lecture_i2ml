...
\lecturechapter{Evaluation: Simple Metrics for Classification}
...
\lz
\begin{enumerate}
\item Class labels $\rightarrow \hh(\xb) = \hat{y}$
...
@
\begin{itemize}
  \item Here: penalize errors on class \textit{versicolor} more heavily
...
\begin{align*}
  Costs &= \meanin C[\yi, \yih] = \frac{1}{\Sexpr{n}} \left( 
