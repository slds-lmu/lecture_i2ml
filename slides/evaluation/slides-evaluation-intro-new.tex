...
\begin{itemize}
  ...
  % \item In practice, we therefore estimate the generalization error based on our 
  % train-test split: 
  % $$\widehat{\mathrm{GE}}_{J_\mathrm{train}, J_\mathrm{test}}(\inducer, 
  % \lambdav, |J_\mathrm{train}|, \rho) = 
  % \rho(\yv_{J_\mathrm{test}}, \bm{F}_{J_\mathrm{test}, \fh}) 
  % \textrm{ with } \fh = \inducer(\Dtrain, \lambdav).$$
\end{itemize}
