...
\lecturechapter{Evaluation: Simple Metrics for Regression}
...
\item $R^2=1$ implies: all residuals are 0, we predict perfectly, \\
$R^2=0$ implies: we predict as badly as a naked constant
