...
\begin{itemize}
  \item Applications where only parts of the ROC curve are relevant to 
  evaluation often occur in biomedical contexts.
  \item For example, 
  ...
  \item Otherwise, healthy patients might receive costly and entirely 
  unnecessary treatment.
  \item Rather than optimizing sensitivity and specificity symmetrically, it is 
  therefore reasonable to demand a certain level of specificity here, and 
...
\begin{itemize}
  ...
  \text{min}}{\text{max} - \text{min}} \right),$$
  where
  \begin{itemize}
    \item $\text{min}$ is the value of the non-discriminant AUC, 
    and
