...
\begin{vbframe}{Choosing Threshold based on ROC Curve}

\begin{itemize}
  \item Identify some (or all) points in ROC space which are not dominated by other points (see blue circles).
  \item Assess TPR and FPR and decide which combination seems best suited for the use-case at hand.
  \item Optimal threshold is mean of thresholds leading to that point.
\end{itemize}

\begin{center}
  \includegraphics[width=0.4\textwidth]{figure/eval_mclass_roc_sp_4a.pdf}
\end{center}
\end{vbframe}
