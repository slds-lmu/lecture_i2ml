...
\begin{itemize}
  \item TRP and FPR often treated asymmetrical in biomedical contexts
  \item TPR, so disease detection, is crucial
...
\begin{itemize}
  ...
  \item Normalized pAUC: percentage of V in V+W,
      map 1 to optimal model, 0 to random model
\end{itemize}
\end{column} 
...
\item Def $\text{AUC}(k ~|~ \ell)$ for classes $k$ (pos) and $l$ (neg)
    % , where $k$ (first index) is pos and $\ell$ neg class

\item Compute AUC: Subset preds to rows of true $k$ and $l$, use $\hat \pi_k$ 
...
\textbf{Example:} AUC(3|1) with $g=3$ 
%\begin{center}
\begin{columns}[T]
...
\item Compute standard AUC with $\hat \pi_3$ as scores\\ %and class $k=3$ as the positive class vs. class $\ell = 1$ as the negative class.\\
\item AUC(3|1) = 1:\\ all pos have higher $\pi_3$ than negs
\end{enumerate}
\end{column}
\end{columns}
...
\begin{itemize}
\item $\text{AUC}(3|1) = 1$ (right table) as before 
