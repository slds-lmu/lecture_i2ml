...
\begin{itemize}
  ...
  % {McClish (1989)}.
  \item If, e.g., an FPR > 0.2 or an TPR < 0.8 is not acceptable for some reasons, we don't care about the performance in that region.
\end{itemize}
...
\begin{itemize}
  \item The scale of the partial AUC depends on the cut-off values used to 
  determine the region of interest $\Rightarrow \text{pAUC} \in [0, c_2 - c_1]$.
  \item For standard AUC: $c_1 = 0$ and $c_2 = 1$.
  \item We can scale pAUC, such that $\text{pAUC} \in [0, 1]$:
