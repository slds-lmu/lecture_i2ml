...
\begin{itemize}
\small
\item A multiclass classifier predicts $\hat \pi_k$, i.e., the probability of belonging to class $k$ for all classes $k \in \gset$.
...
\item For $\text{AUC}(k ~|~ \ell)$,
the first index refers to the positive class (here: $k$) and the second index to the negative class (here: $\ell$).
%\item For $g = 2$, we have $\text{AUC}(k ~|~ \ell) = \text{AUC}(\ell ~|~ k)$. But in general, for $g > 2$, this is not the case.
...
\textbf{Example:} $\text{AUC}(3|1) \neq \text{AUC}(1|3)$ because $\hat \pi_3$ perfectly separates all negative from positive classes at $\hat \pi_3 = 0.25$ (right table) but $\hat \pi_1$ does not (left table).

\begin{center}
\includegraphics[width=0.65\textwidth]{figure_man/multiclass-auc.pdf}
...
\begin{vbframe}{Multi-class AUC}
Hand and Till (2001)
  %\footnote{Hand, D. J., Till, R. J. (2001). A simple generalisation of the area under the ROC curve for multiple class classification problems.}
  proposed to average the AUC of pairwise comparisons (\textbf{one-vs-one}) of a multiclass classifier:
  %\begin{itemize}
...
\begin{itemize}
\item Other definitions are based on \textbf{one-vs-rest} comparisons and average $g$ different AUC values.
\item Imbalanced classes can be considered by weighting individual AUC values based on prior class distributions.
\end{itemize}
