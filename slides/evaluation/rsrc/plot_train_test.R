ggTrainTestPlot = function (data, 
  ...
  gg = ggplot() + 
    geom_line(data = line.data, 
              aes(x = x, y = y, size = "True function"), 
              linetype = "longdash", 
              colour = "grey") +
    scale_size_manual("", 
                      values = 0.5, 
  ...
  if (test.plot) {
    ...
    gg = gg + 
      geom_point(data = data, aes(x = x, y = y, shape = dummy)) +
    ...
    gg = gg + 
      geom_point(data = data[-test.ind, ], aes(x = x, y = y, shape = dummy)) +
      scale_shape_manual(values = c(19, 1), 
                         guide = guide_legend(
                           title = "",
                           override.aes = list(size = 2)))
  ...
  if (! is.null(degrees[1])) {
    ...
    for (d in degrees) {
      ...
      df.poly = rbind(df.poly, data.frame(
        x = x.truth, 
        y = predict(temp.mod, newdata = data.frame(x = x.truth)), 
        degree = d
      ))
      
      train.test.errors[[paste0("degree", d)]] = 
        c(train = mean((data$y[-test.ind] - predict(temp.mod))^2),
          test = mean((data$y[test.ind] - 
                         predict(temp.mod, 
                                 newdata = data.frame(
                                   x = data$x[test.ind])))^2))
    }
    ...
    gg = gg + 
      geom_line(data = df.poly, aes(x = x, y = y, color = degree))
