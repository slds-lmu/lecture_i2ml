...
\newcommand{\titlefigure}{figure_man/eval_fig_title_intro.png}
\newcommand{\learninggoals}{
\item Understand the goal of performance estimation
\item Understand the difference between outer and inner loss
\item Know the definition of generalization error}
...
\begin{itemize}
  ...
  \mapsto \fhDlambda.$$
  \item We obtain $\fhDlambda$ by means of empirical risk minimization, 
...
\begin{itemize}
  ...
  $$\E_{\D\sim(\Pxy)^{\ntrain}}\left[\frac{1}{n}\sumin L\left(\yi, \fhDlambda(\xi)\right)  \right] \xrightarrow{n \to \infty} \GE(\inducer, \lamv, \ntrain, \rhoL) .$$
