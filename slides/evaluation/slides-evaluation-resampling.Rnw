...
\begin{itemize}
  ...
  \item The performance estimates for each fold are dependent, because
...
\begin{itemize}
  ...
  (e.g. detect significant performance differences between methods).
...
\begin{itemize}
  ...
  \item For small data situation with less than 500 or 200 observations, use 
  repeated CV
  \item A $\D$ with $|\D| = 100.000$ can have small sample size properties if one class has only 100 observations 
