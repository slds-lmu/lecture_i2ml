  ...
  \begin{itemize}
    ...
    \item For two data points $\xv$ and $\tilde{\xv}$ with $p$ features $\in \R$
      \begin{itemize}
        ...
        \item the Manhattan distance is % based on the $L_1$-norm:
...
\begin{itemize}
\item Compute for each point the $k$-nearest neighbours in training data $N_k(\xv)$ and average output $y$ of these $k$ neighbors.
\item For regression:
$$
