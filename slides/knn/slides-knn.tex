...
\item The subset of the feature space $\Xspace$ that is as least as close to $\xv$ as its $k$-th closest neighbor $\xi[k]$ in the training data set is called the $k$-\textbf{neighborhood} $N_k(\xv)$ of $\xv$:
\begin{footnotesize}
$$N_k(\xv) = \{ \xi \in  \Xspace \mid d(\xi, \xv) \leq d(\xi[k], \xv)\}$$
\end{footnotesize}
\end{itemize}
