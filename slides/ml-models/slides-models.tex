...
\begin{itemize}
    \item \textbf{R:} package for regularized linear model \code{glmnet}
    \item \textbf{Python:} \code{LinearRegression} from package 
    \code{sklearn.linear\_model}, package for advanced statistical parameters 
    \code{statsmodels.api} 
  \end{itemize}
...
\medskip
  ...
  \highlight{Implementation} 
  \begin{itemize}
    \item \textbf{R:} package for regularized linear model \code{XXX}
...
\highlight{General idea} 
\begin{itemize}
  \item \bfit{Random forests (RF)} are \textbf{bagging ensembles}: they combine 
  $M$ CART (base learners) to form a strong learner. 
  \item They use \textbf{complex} trees with low  bias and compensate for single 
  trees' high variance by aggregating them in a \textbf{decorrelated} manner. 
  ...
  \item Aggregation takes place via \textbf{averaging} (regression) or 
...
\begin{itemize}
  ...
  \textbf{layers}. These layers are assembled through functional connections. 
  \item Data enter the network in the \textbf{input layer} and sequentially pass 
...
\begin{columns}[onlytextwidth]
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
      ...
      \positem can easily be updated %batch propagation
...
\conclbox{Computationally expensive models which can learn complex functions and are suitable for unstructred data like text or pictures.}
