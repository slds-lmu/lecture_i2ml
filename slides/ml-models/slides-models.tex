...
%\begin{itemize}
A linear model (LM) depends on a weighted sum of the input variables and includes a large body of models. 
...
\begin{itemize}
  \item identity function $\phi(\thetab^\top \xv) = \thetab^\top \xv$ :  \textbf{linear regression} - fit a \textbf{hyperplane} $\theta_0 + \thx$ for continous values
  \item logistic sigmoid function $\phi(\thetab^\top \xv) = \frac{1}{1 + \exp(- \thetab^\top \xv)}$ : \textbf{logistic regression} - fits a wave based on the logit link fucntion (inverse of logistic sigmoid function). Applying a descion rule ($>0.5$) results in a sparating hyperplane. 
