...
%\item Support vector machines (SVMs) construct separating \textbf{hyperplanes} in a multi-dimenstional space.  
  ...
  %\item It is a \textbf{convex optimization problem} (quadratic programm) which tries to maximise the \enquote{safety margin} where the distance of all data point to the hyperplane should be greater than the margin. 
...
\conclbox{Very accuarate solution for high-dimensional data that is linear separable}
...
\conclbox{Non-linear SVMs perform very well for non-linear separable datas, but are hard to interpret and need a lot of tuning.}
...
\medskip
 ...
 \begin{itemize}
    ...
    \item For the RBF kernel the the \textbf{RBF sigma heuristic} is used. 
  \end{itemize}
