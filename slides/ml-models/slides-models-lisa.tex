...
\textbf{\textcolor{gray!80}{Empirical risk for typical loss functions}} \\

\begin{itemize}
  \item Empirical risk is calculated per terminal node $\Np_m$.
...
to form a strong learner. They use \textbf{complex}, fully grown trees with low 
bias and compensate for single trees' high variance by aggregating $M$ of them 
in a \textbf{decorrelated} manner. 
...
\begin{itemize}
  ...
  \begin{itemize}
    \footnotesize
    \item Using \textit{\textbf{Brier score}} \\
    $\risk(\Np_m) = \sum\limits_{(\xv,y) \in \Np_m} \sumkg \left( \I(y = k)
    - \pikx \right)^2$
    \item Using \textit{\textbf{Bernoulli loss}} \\
    $\risk(\Np_m) = \sum\limits_{(\xv,y) \in \Np_m} \sumkg \I(y = k) \cdot
    \log(\pikx)$
  \end{itemize}
  \item Regression: Using \textit{\textbf{quadratic loss}} \\
  $\risk(\Np_m) = \sum\limits_{(\xv,y) \in \Np_m} (y - c_m)^2$
\end{itemize}
