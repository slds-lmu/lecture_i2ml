...
%\begin{itemize}
\textcolor{blue}{A linear model (LM) represents the target as a linear combination of the input variables.}
...
\begin{itemize}
  \item identity function $\phi(\thetab^\top \xv) = \thetab^\top \xv$ :  \textbf{linear regression} - \textcolor{blue}{a \textbf{hyperplane} $\theta_0 + \thx$ is fitted to the data} (regression task).
  \item logistic sigmoid function $\phi(\thetab^\top \xv) = \frac{1}{1 + \exp(- \thetab^\top \xv)} = \pix$ : \textbf{logistic regression} - the probability $\pix = \P(y = 1 ~|~\xv)$ is fitted by a \textcolor{red}{wave} based on the logit link (inverse of logistic sigmoid function). Applying a decision rule (\textcolor{blue}{e.g., $\pix >0.5$}) results in a separating hyperplane (classification task, linear classifier). 
...
\begin{itemize}\footnotesize
  ...
  \item For \textbf{logistic regression} the risk function is based on the \textbf{Bernoulli loss} $\risket = \textcolor{blue}{\sumin} -\yi \log \left(\pixii\right) - (1 - \yi) \log \left(1 - \pixii \right)$.
...
\begin{itemize}\footnotesize
  \item for \textbf{OLS}: analytically with $\bm{\hat \theta} = (\Xmat^\top \Xmat)^{-1} \Xmat^\top\ydat$ \\ \textcolor{blue}{($\Xmat \in \R^{n \times p}$ : matrix of feature vectors)}
...
\begin{columns}[onlytextwidth]
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
      ...
      \negitem \textcolor{red}{\textbf{strong assumptions}: data is independent (multi-collinearity must be removed)}
...
\footnotesize
 ...
 \textcolor{red}{This model is very effective if the following assumptions are fulfilled:}
 \begin{itemize}\footnotesize
  \item \textbf{linearity}: \textcolor{blue}{The expected response is a linear combination of the features.}
...
\item We can find a compromise between generalizing the model (simple model, underfitted) and \textcolor{blue}{corresponding} closely to the data (complex model, overfitted).
...
\item Therefore, we minimize the empirical risk function $\risket$ \textbf{plus \textcolor{blue}{the} a complexity \textcolor{blue}{penalty $J(\thetab)$, controlled by shrinkage parameter $\lambda$}} :
  $
  \riskrt = \risket + \lambda \cdot J(\thetab). 
  $ 
  
\item \textbf{Ridge regression} uses the L2-penalty as a complexity measure with $J(\thetab) = \textcolor{blue}{\|}\thetab\|_2^2 $. 

\item Alternativly, \textbf{LASSO} (least absolute shrinkage and selection operator) uses the L1-penalty ($J(\thetab) = \textcolor{blue}{\|}\thetab\|_1 $).

\item \textcolor{blue}{While} both regularization methods shrink the coefficients of the model; LASSO also performs \textbf{feature selection}. 

\item \textbf{Elastic net} uses a convex combination of Ridge and LASSO with $J(\thetab) = \textcolor{blue}{+} \lambda_1 \|\thetab\|_1 + \lambda_2 \|\thetab\|_2^2$.
...
\textcolor{red}{ALMOST SAME LIKE LINEAR MODEL?--> does this make sense?}
...
\begin{columns}[onlytextwidth]
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
      ...
      \positem basis for many machine learning algorithms
      \positem \textcolor{red}{less tendency to \textbf{overfit}}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
      ...
      \negitem \textcolor{red}{\textbf{strong assumptions}: data is independent (multi-collinearity must be removed and normal distributed residuals ??}
...
%\item Support vector machines (SVMs) construct separating \textbf{hyperplanes} in a multi-dimenstional space.  
  ...
  \item The support vector machine (SVM) algorithm finds a decision boundary (\textbf{separating hyperplane}) that maximizes the distance (\textbf{margin $\gamma$}) \textcolor{blue}{to} the closest members (\textbf{support vectors, SV}) of the separate classes. (\textbf{hard margin})
...
\highlight{Hyperparameters} ~~ \textcolor{blue}{$C$} : penalization for missclassified data points 


\medskip
%https://www.ijcaonline.org/research/volume128/number3/abdiansah-2015-ijca-906480.pdf
\highlight{Runtime behavior} ~~ \textcolor{red}{$\mathcal{O}(n^3)$ for $n$ observations} %$\mathcal{O}(n^2 \cdot p + n^3)$ for $n$ observations and $p$ features
...
\begin{columns}[onlytextwidth]
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}
      ...
      \negitem does not scale well to \textbf{larger data sets} \textcolor{red}{\textbf{??}}
...
\conclbox{Very accurate solution for high-dimensional data that is \textcolor{blue}{linearly} separable}
...
\begin{itemize}
  \item \textcolor{blue}{$C$}: penalization for missclassified data points 
  \item \textbf{Kernel parameters}: depending \textcolor{blue}{on which kernel (if any)} is used (e. g. degree of the polynomial kernel or width of RBF kernel)
