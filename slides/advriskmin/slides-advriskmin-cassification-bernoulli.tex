...
The risk minimizer for the Bernoulli loss defined for probabilistic classifiers $\pix$ and on $y \in \{0, 1\}$
...
\textbf{Proof:} Again, we make use of the law of total expectation
...
with $\eta(\xv) = \P(y = 1 ~|~ \xv = \xv)$. We fix a specific $\xv$ and compute the point-wise optimal value $c$ by setting the derivative to $0$: 
...
The risk minimizer for the Bernoulli loss defined on $y \in \{0, 1\}$ and scores $\fx$ is the point-wise log-odds:
...
\textbf{Claim:} Entropy splitting is equivalent to the log loss minimization. 
...
$$
  ...
  where $I$ is the entropy criterion $I(\Np) = -\sum_{k=1}^g \pikN \log \pikN,$



  and $\risk(\Np)$ is calculated w.r.t. the log loss 

$$
  L(y, \pikx) = \sum_{k = 1}^g \I(y = k) \log \left(\pi_k(\xv)\right).
$$

\framebreak 

\begin{eqnarray*}
\risk(\Np) &=& \sum_{\xy \in \Np}  \sum_{k = 1}^g \I(y = k) \log \pi_k(\xv) \overset{(*)}{=} \sum_{k = 1}^g \sum_{\xy \in \Np} \I(y = k)\log \pikN \\
&=& \sum_{k = 1}^g \log \pikN \underbrace{\sum_{\xy \in \Np} \I(y = k)}_{n_{\Np}\cdot \pikN } \\
 &=& n_{\Np} \sum_{k = 1}^g \pikN \log \pikN = n_\Np I(\Np), 
\end{eqnarray*} 

where in $^{(*)}$ the optimal constant $\hat \pi_k(\xv) = \pikN = \frac{1}{n_{\Np}} \sum\limits_{(\xv,y) \in \Np} \I(y = k)$ was plugged in. 
% \framebreak
