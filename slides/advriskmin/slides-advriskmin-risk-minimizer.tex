...
But since we usually do not no $\Pxy$, consistency
...
In Stone's famous consistency theorem from 1977, the universal consistency a weighted average estimator as KNN was proven. Many other ML model have since then been proven to be universally consistent (SVMs, ANNs, etc.).
...
\only<2>{
    For the 0-1 loss, we the risk minmizer construct the \textbf{optimal Bayes decision rule}: We predict the class with maximal posterior probability.
}
\only<3>{
