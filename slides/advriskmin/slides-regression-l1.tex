...
% https://math.stackexchange.com/questions/113270/the-median-minimizes-the-sum-of-absolute-deviations-the-l-1-norm/1024462#1024462
  ...
  W.l.o.g. asssume now that all $\yi$ are sorted in increasing order.
  ...
  \sum\limits_{i = 1}^{i_{\max}} \left(S_{\yi, \yi[n + 1 - i]}(\theta)\right) + |\yi[n + 1] - \theta| & \text{ for } n \text{ is odd}. \end{cases}
  \end{eqnarray*}
  \end{footnotesize}
  ...
  \item for \enquote{$n$ is odd}: $\thetah = y^{(n+1)/2} \in \mathcal{I}_{i_{\max}} = \{y^{(n+1)/2}\}$ minimizes $S_i$ for all $i \in \{1,\dots, i_{\max}\} \; \Rightarrow$ it minimizes $\riske$,
  \end{itemize}
