...
\newcommand{\titlefigure}{figure/fig-regu-nonlin-4.png}
...
%-------------------------------------------------------------------------------
%-------------------------------------------------------------------------------
\begin{frame}{Non-linear Risk Minimization: Example}
...
$$
- \sumin \log p\left(\xi ~|~ \thetab\right) - \textcolor{blue}{\lambda} 
\log q(\thetab).
$$
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $q(\thetab) = \prodjp \mathcal{N}(0, \tau^2)
    \propto \exp \left( \frac{1}{2\tau^2} \sumjp \theta_j^2 \right),$ and
  \item $- \log q(\thetab) \propto -\log \left( \exp \left( \frac{1}{2\tau^2} 
  \sumjp \theta_j^2 \right)\right) \propto \sumjp \theta_j^2.$
  \end{itemize}
  \item The conditional distribution of $y ~|~ \xv$ in linear regression is
  also Gaussian:
  \begin{itemize}
    \item $p\left(\xv ~|~ \thetab\right) \propto \exp \left( \frac{1}{2\sigma^2} 
    \sumin \left(\yi - \fxit \right)^2 \right),$ and
  \item $-\log p\left(\xv ~|~ \thetab\right) \propto \sumin \left(\yi - \fxit 
  \right)^2.$
  \end{itemize}
  \item With this, the corresponding MAP estimation problem becomes:
  \begin{eqnarray*}
    \thetah^{\text{MAP}} &=& \argmin_{\thetab} \left(
    - \sumin \log p\left(\xi ~|~ \thetab\right) - \lambda \cdot \sumjp \log
    q(\theta_j)
    \right) \\
    &=& \argmin_{\thetab} \left(
    \sumin \left(\yi - \fxit \right)^2 + \lambda \sumjp \theta_j^2
    \right) \\
    &=& \argmin_{\thetab} \left(\ydat - \Xmat \thetab\right)^\top \left(\ydat - 
    \Xmat \thetab\right) + \lambda {\thetab}^T\thetab \\
    &=& (\Xmat^T \Xmat  + \lambda \id)^{-1} \Xmat^T\ydat \\
    &=& \thetah^{\text{Ridge}}.
  \end{eqnarray*}
\end{itemize}
...
% ------------------------------------------------------------------------------
  ...
  % \item We derive $\thetah_{\text{MAP}}$ as follows:
  % \item Recall the Ridge estimator
  % \begin{eqnarray*}
  %   \thetah_{\text{Ridge}} &=& \argmin_{\thetab} \left(\ydat - \Xmat 
  %   \thetab\right)^\top \left(\ydat - \Xmat \thetab\right) + \lambda 
  %   \thetab^\top \thetab \\
  %   &=& (\Xmat^T \Xmat  + \lambda \id)^{-1} \Xmat^T\ydat.
  % \end{eqnarray*}
% \end{itemize}
