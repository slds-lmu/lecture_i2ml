...
plotModQuadraticLoss = function (data, model, loss, pt_idx) {
  ...
  df_plot = data.frame(x = x[pt_idx], 
                       y = y[pt_idx], 
                       y_hat = predict(model)[pt_idx])
  ...
  gg1 = ggplot() +
    ...
    geom_rect(data = df_plot, 
              aes(ymin = y_hat, ymax = y, xmin = x, xmax = x + resid), 
              color = "black", 
              linetype = "dotted", 
              fill = "red", 
              alpha = 0.2) +
    ...
    geom_text(data = df_plot, 
              aes(x = x + resid / 2, y = y + resid / 2, label = loss_label), 
              color = "red") +
  coord_fixed()
  ...
  gg2 = ggplot() +
    ...
    geom_point(data = df_plot, 
               ...
               color = "red", size = 2) +
    geom_segment(data = df_plot, 
                 mapping = aes(x = resid, y = 0, xend = resid, yend = loss), 
                 color = "red") +
    xlab(expression(Residuals == y - hat(y))) + 
    ylab(expression(L(hat(y), y))) +
    geom_text(data = df_plot, 
              mapping = aes(x = resid, y = loss, label = loss_label), 
              color = "red", 
              hjust = -0.5)
...
plotModAbsoluteLoss = function (data, model, pt_idx, add_quadratic = TRUE) {
  ...
  gg1 = ggplot() +
    ...
    geom_point(data = df_plot, 
               ...
               color = "red", size = 2) +
    geom_text(data = df_plot, 
              aes(x = x, y = y + resid / 2, label = loss_label), 
              color = "red", 
              hjust = -0.5) +
  coord_fixed()
  ...
  if (add_quadratic) {
    ...
    gg2 = gg2 +
      stat_function(data = data.frame(x = c(-4, 4)), 
                    mapping = aes(x), fun = function (x) {x^2}, alpha = 0.4) +
      geom_point(data = df_plot, 
                 aes(x = resid + 0.1, y = loss_qu), 
                 color = "red", 
                 size = 2, 
                 alpha = 0.4) +
      geom_segment(data = df_plot, 
                   mapping = aes(x = resid + 0.1, 
                                 y = 0, 
                                 xend = resid + 0.1, 
                                 yend = loss_qu), 
                   color = "red", 
...
plotModAbsLogQuadLoss = function (data, 
                                  model, 
                                  pt_idx, 
                                  add_quadratic_abs = TRUE) {
  ...
  gg1 = ggplot() +
    ...
    geom_segment(data = df_plot, 
                 aes(x = x, 
                     y = y_hat, 
                     xend = x, 
                     yend = y_hat + sign(y - y_hat) * loss), 
                 color = "red") +
    geom_point(data = df_plot, aes(x = x, y = y), color = "red", size = 2) +
    geom_text(data = df_plot, 
              aes(x = x, 
                  y = y_hat + sign(y - y_hat) * loss / 2, 
                  label = loss_label), 
              color = "red", 
              hjust = -0.5) +
  coord_fixed()
