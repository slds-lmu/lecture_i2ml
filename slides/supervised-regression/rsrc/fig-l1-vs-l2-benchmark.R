...
pdf("../figure/l1_vs_l2_benchmark_plot.pdf", width = 7, height = 3.5)
plt_loss <- ggplot(df_mbm, aes(x=loss_dataset, y=log(time), fill=loss)) +
  stat_boxplot(geom = "errorbar", width = 0.5) +
  geom_boxplot() + labs(x ="Proportion of dataset", y = "Log. time in ms") +
  scale_x_discrete(labels=c("10%","10%", "50%", "50%", "100%", "100%")) +
  theme(legend.position="bottom") +
  guides(fill=guide_legend(title="Loss"))
plt_loss
ggsave("../figure/l1_vs_l2_benchmark_plot.pdf", plt_loss, width = 7, height = 3.5)
