...
\begin{itemize}
\item Dataset: \textbf{dailybike} (OpenML id = 45103)
\item formula: $rentals_i = \Sigma_{j = 1}^d \theta_j temp_i^{j} + \theta_{d + 1} season_i +  \theta_{d + 2} hum_i +  \epsilon_i$, with d = degree of polynomial.
\end{itemize}

\
\includegraphics[width=0.6\textwidth]{figure/polynomial_regression_test_train_error}
\includegraphics[width=0.3\textwidth]{figure/polynomial_regr_effect_plot_degree_3}
\

(Why is evaluation on a test set necessary? see lecture slides on evaluation)
