...
\item Understand how a tree is grown}
...
For regression and L2 loss, the mean in each leaf is optimal:

\end{column}
\begin{column}{0.5\textwidth}
For (hard-label) classification and Bernoulli / Multinomial loss, the majority label is optimal:
\end{column}
\end{columns}
...
\begin{column}{0.5\textwidth}
\begin{figure}
\includegraphics[width=0.5\textwidth]{figure/splitcrit_optimal-constant.pdf} 
\end{figure}

\end{column}
\begin{column}{0.5\textwidth}
\begin{figure}
\includegraphics[width=0.55\textwidth]{figure/splitcrit_optimal-constant-classif.pdf} 
\end{figure}
\end{column}
\end{columns}\lz
(For probabalistic classification we choose the rel. frequency of a label.)
...
When splitting the data into leafs, we can use the idea of the "optimal constant model" in each leaf. E.g. consider these two splits:
...
The total sum of individual risks yields:
\vspace{0.1in}
...
Based on the variance, we would prefer the first split.
In fact, minimizing the variance in each leaf is equivalent to minimizing L2 loss.
If we want to find the split with the lowest variance, we need to search the domain of $x_j$.
