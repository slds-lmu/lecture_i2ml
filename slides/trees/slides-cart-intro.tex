...
The tree can also be expressed as a set of nested if-else statements:

\begin{columns}
\begin{column}{0.5\textwidth}
\begin{algorithmic}
\If{\texttt{Sepal.Length} $< 5.5$} 
   \If{\texttt{Sepal.Width} $\geq 2.8$}
     \State Predict 
...
Trees divide the feature space $\Xspace$ into \textbf{rectangular regions}: 
  ...
  \begin{figure}
\includegraphics[width=0.7\textwidth, keepaspectratio]{figure/tree-depth3-area-withblacklines.pdf}
\end{figure}
...
For a probabalistic classification outcomes, our prediction surface looks similar, yet with $\pix$ \in [0,1]$. 
