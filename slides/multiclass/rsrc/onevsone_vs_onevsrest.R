  ...
  # df = read.csv(file = "rsrc/results.csv")
  ...
  p1 = ggplot(bla, aes(x = g, y = timetrain, colour = method)) + geom_line()
  p1 = p1 + geom_line(data = bla, aes(x = g, y = nclassifs / 5, colour = method), lty = 2)
  p1 = p1 + scale_y_continuous("timetrain", 
      sec.axis = sec_axis(~ . * 5, name = "# classifiers trained")
    )

ggsave("figure_man/onevsone_vs_onevsrest.png", p1, width = 5, height = 3)
