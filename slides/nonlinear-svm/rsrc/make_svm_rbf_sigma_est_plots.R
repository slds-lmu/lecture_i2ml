# ------------------------------------------------------------------------------
# FIG: SVM RBF KERNEL SIGMA ESTIMATION PLOTS
# ------------------------------------------------------------------------------
...
set.seed(1234)
makeSimpleSimulatedTask <- function(size) {
  x1 <- rnorm(size, mean = 0)
  x2 <- rnorm(size, mean = 0)
...
tsk <- makeSimpleSimulatedTask(10)
...
plotKernelFunctions <- function(tsk, sigma) {
  ...
  g <- ggplot() +
    # geom_point(aes(x = x1, y = x2, colour = y), data = df, size = 4) +
...
ggsave("../figure/svm_rbf_sigma_est_narrow.pdf")

plotKernelFunctions(tsk, 1.5)
ggsave("../figure/svm_rbf_sigma_est_wide.pdf")


# Plotting the histogram

tsk <- makeSimpleSimulatedTask(50)
...
ggsave("../figure/svm_rbf_sigma_est_hist.pdf")
