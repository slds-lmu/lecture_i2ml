...
\begin{itemize}
  \item Let $\text{OOB}^{[m]}$ denote the index set $$\left \{i \in \setn: \xyi 
  \text{ is OOB for } \blm \right \}$$ of OOB observations in 
  $\blm$, and let $\hat{y}^{(i)[m]}$ be the prediction for observation $i$ 
  by $\blm$.
  \item Compute the OOB prediction for each observation:
  $$\yih_{\text{OOB}} = \begin{cases}
  \frac{1}{|\{m \in \{1, \ldots, M\}: i \in 
  \text{OOB}^{[m]} \}|} \sum_{m = 1}^{M} 
  \I(i \in \text{OOB}^{[m]}) \cdot \hat{y}^{(i)[m]} \quad \text{in regression,} \\
  \phantom{x} \\
  \argmax_{k \in \setg} \sum_{m = 1}^{M} \I(i \in \text{OOB}^{[m]}) \cdot
   \I(\hat{h}^{(i)[m]} = k) \quad \text{in classification.}
  \end{cases}$$
  ...
  $i \in \setn$. \\
  $\rightarrow$ Similar to 3-CV, can be used for a quick model selection.
\end{itemize}
