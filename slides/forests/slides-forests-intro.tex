...
% THIS PAGE IS ONLY FOR SOME LATEX CODE THAT HAS BEEN INSERTED INTO PLOT ON 
% FOLLOWING PAGE PER SCREENSHOT
% this is extremely ugly but currently the best viable solution
% if the text needs to be modified, this can be un-commented, rendered once, 
% and inserted into the picture (which is in google drawings, link below)
...
% 
% \begin{split}
% \yh^{(2)}_{\text{OOB}} &= \frac{1}{\textcolor{orange}{S_{\text{OOB}}^{(2)}}} 
...
% Source: https://docs.google.com/drawings/d/1Lfc7lRDgtrkwHDRlJahDNYtOoQ7biBJQkGGTjMDkeaM/edit
\includegraphics[width = 0.95\textwidth]{figure_man/forests-oob-pred}
...
\begin{itemize}
  ...
  % \item As an example for the latter case, assume a classification task with 
  % $g = 3$ classes where observation $i$ is OOB in $S_{\text{OOB}}^{(i)} = 10$ 
  % trees. If classes 1 to 3 are predicted 4, 4 and 2 times, respectively, we have 
