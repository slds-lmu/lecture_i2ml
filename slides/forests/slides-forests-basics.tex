...
Since bootstrap samples are similar, trained base learners $\blh$ are correlated, affecting the variance of an ensemble $\fh$. Assuming $\var(\blh) = \sigma^2$, $\cov(\blh, \blh[j]) = \rho$:
\begin{align*}
...
$\Rightarrow \var\left(\fh\right)$ should be small to minimize MSE \\
$\Rightarrow$ Random forests reduce $\cov(\blh, \blh[j])$, hence reduce $\var\left(\fh\right)$%, leading to improved model performance
\end{vbframe}
...
\begin{vbframe}{Can an RF overfit? \citebutton{P. Probst and A. -L. Boulesteix, 2018}{https://jmlr.org/papers/v18/17-269.html}}
