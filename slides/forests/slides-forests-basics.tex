...
\begin{itemize}
  ...
  \item \textbf{Invariance to linear transformations}: Their performance is unaffected by scaling or shifting of features.
...
% \vspace{1em}
% An extension of bagging creates the random forest (RF), which \textbf{sacrifices interpretability} for significant performance gains.
\end{vbframe}
...
\item \textbf{Random feature sampling} decorrelates the base learners. % and improves model performance. %Since bootstrap samples are largely similar, the trained base learners $\blh$ tend to be correlated. RFs reduce the correlation between individual trees, leading to improved model performance.
...
RFs decorrelate trees with a randomized approach:
