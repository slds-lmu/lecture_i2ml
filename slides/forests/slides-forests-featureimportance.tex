...
\begin{center}
% FIGURE SOURCE: https://docs.google.com/drawings/d/1Wx_XANPl6bMS8NqmO5W4EgvnCqmOmgyxU5OmUGkEvNo/edit
\includegraphics[width = 0.7\textwidth]{figure_man/forests-featimp}
\end{center}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[0]
    ...
    \For{features $x_j$, $j = 1$ to $p$}
    % \For{observations $\xyi$, $i = 1$ to $n$}
    %   % \State {Find all trees $m^{\prime}$ that use $x_j$ and for which $\xyi$ 
    %   % is OOB.} 
    %   \State {Find trees for which $\xyi$ is OOB.}
    %   \State {Compute OOB predictions $\yih_{\text{OOB}}$.}
    %   \State {Compute corresponding loss $L(\yi, \yih_{\text{OOB}})$.}
    % \EndFor
      \State {Perform permutation $\psi_j$ on $x_j$ to distort feature-target 
      relation for $x_j$.}
    \For{distorted observations $\left(\mathbf{x}^{(i)}_{\psi_j}, \yi\right)$, 
    $i = 1$ to $n$}
      % \State {Find trees for which $\xyi$ is OOB.}
