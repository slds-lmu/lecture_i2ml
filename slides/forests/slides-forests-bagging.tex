...
\begin{vbframe}{Bagging Pseudo Code}
  % TODO x wegnehmen; dafür zwischen hard label classifier etc. unterscheiden
\vspace{-3ex}
\begin{algorithm}[H]
  \scriptsize
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \State {\bf Input: } Observation $\xv$, trained base learners $\blh$ (either hard label classifier $\hat{h}^{[m]}$ or probability estimator $\hat \pi^{[m]}$)
