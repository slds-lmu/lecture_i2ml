...
\begin{itemize}
  ...
  \item Each base learner is trained on a different \textbf{bootstrap} sample from the original dataset $\D$.
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \begin{center}
    $\quad \hxh = \argmax_{k \in \Yspace} \sum_{m=1}^M \I\left(\hat{h}^{[m]} = k\right) \textit{(majority voting)}$
    $ \pikxh= \fMh = \frac{1}{M} \sum_{m=1}^M \blh \textit{(probabilities through averaging)}$
    $\pikxh= \sum_{m=1}^M \I\left(\hat{h}^{[m]} = k\right) \textit{(probabilities through class frequencies)}$
    \end{center}
	% \vspace{-3ex}
	%     \begin{align*}
	%     \quad \hxh^{[M]} &= \argmax_{k \in \Yspace} \sum_{m=1}^M \I\left(\blh = k\right) \textit{(majority voting)}\\[-2ex]
	%      \pikxh &= \fMh = \frac{1}{M} \sum_{m=1}^M \blh \textit{(probabilities through averaging)}\\[-2ex]
