...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \State {\bf Input: } Observation $o$, trained base learners $\blh$
...
\begin{itemize}
  \item \textbf{Benefits of larger ensembles}: Increasing the \textbf{number of base learners} improves performance (at least theoretically): % hierfür Grafik: Ensemble Size, Fehler; mit Benchmark oder Tuner (design search mlr3)
\begin{center}
\includegraphics[width=0.55\textwidth]{figure/bagging-ntree_MSE.png}
\end{center}
  \item \textbf{Advantages of diversity}: Diverse base learners contribute different perspectives, reducing the impact of any single model's bias or variance. % das als motivation für RF-intro
  % \item \textbf{Less correlation, more gain}: Bagging yiels better results when there is less correlation among the models. Independent errors help in cancelling out individual errors! Dieser Teil im RF Teil!!
\end{itemize}
