...
\begin{itemize}
  ...
  \item Random Forests as combinations of trees lose this feature
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[0]
    ...
    \State {Compute improvement in splitting criterion for each.}
    \State {Sum these up.}
    \EndFor
    \State {Sum up the accumulated improvement over all trees.}
