...
In supervised ML, we want to learn the function \(f\) \textbf{automatically from labeled data}.\\
We call this function a \textbf{model}.
...
A \textbf{model} (or hypothesis) $f : \mathcal{X} \rightarrow \mathcal{Y}$ is a function that maps feature values to target values (labels).\\
...
So: a model takes in feature vectors and turns out predictions for a target variable. 
\end{frame}
...
\item and produces a \textbf{model}, i.e., a function $f:\, \mathcal{X} \to \mathcal{Y}$ from a hypothesis space $\Hspace$.\\
\end{itemize}
