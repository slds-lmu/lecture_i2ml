...
plotModel = function(m, x1, yhat, r, rhat, beta) {
  ...
  p1 = ggplot() + geom_point(aes(x = x1, y = y), alpha = .5) +
    geom_step(aes(x = xgrid, y = yhatgrid)) +
  ...
  p2 = ggplot() + geom_point(aes(x = x1, y = r), alpha = .5) +
    geom_step(aes(x = xgrid, y = rhatgrid)) +
...
p1 = ggplot(data = data.frame("x" = x,"y" = y), aes(x,y)) + geom_point()
p2 = ggplot(data = data.frame("x" = x,"y" = y), aes(x,y)) + geom_point() + geom_hline(aes(yintercept = mean(y)))

ggsave("figure_man/gbm_anim_data.png", p1, width = 5, height = 4)
ggsave("figure_man/gbm_anim_init.png", p2, width = 5, height = 4)

for (nboost in c(1:11, 31)) {
  png(file = sprintf("figure_man/gbm_anim_%02i.png", nboost))
  plotGBMIteration(nboost)
  dev.off()
}
