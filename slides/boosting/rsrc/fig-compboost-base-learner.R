...
# - one univariate spline ()
# - one univariate categorical feature ()
# - one tensor product spline (Longitude - Latitude)
n_iters = 1000L

# Only linear base learners

set.seed(31415)
cboost = Compboost$new(data = dat, target = "Sale_Price", learning_rate = 0.01,
  loss = LossQuadratic$new(), oob_fraction = 0.2)

cboost$addBaselearner("Fireplaces", "linear", BaselearnerPolynomial)
cboost$addBaselearner("Year_Built", "spline", BaselearnerPSpline, df = 8)
cboost$addBaselearner("Overall_Qual", "ridge", BaselearnerCategoricalRidge, df = 2)
cboost$addTensor("Longitude", "Latitude", df = 2)
...
gg0 = plotPEUni(cboost, "Fireplaces") +
  ...
  ggplot2::ylab("b(Fireplaces)")

gg1 = plotPEUni(cboost, "Year_Built") +
  ggplot2::theme_minimal() +
  ggplot2::scale_color_brewer(palette = "Dark2", name = "base learner") +
  ggplot2::ylab("b(Year_Built)")

gg2 = plotPEUni(cboost, "Overall_Qual") +
  ggplot2::theme_minimal() +
  ggplot2::scale_color_brewer(palette = "Dark2", name = "base learner") +
  ggplot2::ylab("b(Overall_Qual)") +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1))

gg3 = plotTensor(cboost, "Longitude_Latitude_tensor", nbins = 7L) +
  ggplot2::theme_minimal() +
  ggplot2::scale_color_brewer(palette = "Dark2", name = "base learner") +
  ggplot2::labs(fill = "b(Longitude, Latitude)")

ggplot2::ggsave(here::here("slides/boosting/figure/compboost-base-learner-linear.png"), gg0, height = 2.2, width = 7L)
ggplot2::ggsave(here::here("slides/boosting/figure/compboost-base-learner-spline.png"), gg1, height = 2.2, width = 7L)
ggplot2::ggsave(here::here("slides/boosting/figure/compboost-base-learner-ridge.png"), gg2, height = 2.2, width = 7L)
ggplot2::ggsave(here::here("slides/boosting/figure/compboost-base-learner-tensor.png"), gg3, height = 2.2, width = 7L)
