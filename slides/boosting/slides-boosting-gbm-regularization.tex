...
Note: If $\alpha^{[m]}$ is found via line search, we multiply the next base learner by an additional constant \textbf{shrinkage parameter} $\nu \in (0,1]$ to shorten the step length.
In the case of a (commonly used) constant learning rate $\alpha$, $\nu$ can be neglected by choosing a smaller value for $\alpha$.
Hence, on the following slides, we call $\alpha$ the shrinkage parameter or learning rate.
