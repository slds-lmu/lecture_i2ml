...
ggsave("figure_man/training/datapoints.pdf", p1, height = 3, width = 4)
...
ggsave("figure_man/training/fit-vs-penalty.pdf", p2, height = 3, width = 4)
...
for (l in c(0.2, 0.5, 2)) {
	...
	ggsave(paste0("figure_man/training/fit-vs-penalty-", gsub("\\.", "_", l), ".pdf"), p2n, height = 3, width = 4)

	p1n = p1 + geom_line(data = res, aes(x = x, y = mpost), colour = "grey")
	p1n = p1n + ggtitle(paste0("l = ", l))
	ggsave(paste0("figure_man/training/datapoints-", gsub("\\.", "_", l), ".pdf"), p1n, height = 3, width = 4)
}