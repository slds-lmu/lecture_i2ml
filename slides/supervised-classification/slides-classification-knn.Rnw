...
\begin{vbframe}{KNN: From small to large k}
...
\begin{vbframe}{KNN as non-parametric model}
\begin{itemize}
\item KNN has no real training step, it simply stores the complete data 
...
\item As number of parameters are growing with the number of training points, we call 
  KNN a non-parametric model
\item Hence, KNN is not based on any distributional or strong functional assumption,
