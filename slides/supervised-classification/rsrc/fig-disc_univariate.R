...
plot(x, dnorm(x, mean = 170, sd = 4), type = "l", col = "red", 
     xlim = c(150, 200), ylim = c(0, 0.1),
     ylab = "f(x)", xlab = "x (body height in cm)")
abline(v = 175, col = "grey", lty = 2)
axis(1, at = seq(155, 195, by = 10))
lines(x, dnorm(x, mean = 180, sd = 4)) 
legend("topright", legend = c("woman ~ N(170, 4)", "man ~ N(180, 4)"), 
...
plot(x, y.woman, type = "l", col = "red", xlim = c(150, 200), ylim = c(0, 0.1), 
     ylab = "f(x)", xlab = "x (body height in cm)")
axis(1, at = seq(155, 195, by = 10))
lines(x, y.man) 
legend("topright", legend = c("woman ~ N(170, 4)", "man ~ N(180, 4)"), 
...
text(155, dnorm(172, mean = 170, sd = 4), pos = 3, col  = "red",
     labels = paste0("f(x|y = woman) = ", woman <- round(dnorm(172, mean = 170, sd = 4), 4)))

lines(c(0, 172, 172), c(rep(dnorm(172, mean = 180, sd = 4), 2), -1), 
      col = "black", lty = 2)
text(155, dnorm(172, mean = 180, sd = 4), 
     labels = paste0("f(x|y = man) = ", man <- round(dnorm(172, mean = 180, sd = 4), 4)), 
     pos = 3, col  = "black")
dev.off()
...
x = seq(150, 200, length.out = 100)
y.woman = dnorm(x, mean = 170, sd = 4)*0.6*2
y.man = dnorm(x, mean = 180, sd = 4)*0.4*2
...
f = function(x) (dnorm(x, mean = 170, sd = 4)*0.6 - dnorm(x, mean = 180, sd = 4)*0.4)
...
legend("topright", legend = c("woman ~ N(170, 4)", "man ~ N(180, 9)"), 
       col = c("red", "black"), lty = 1)
