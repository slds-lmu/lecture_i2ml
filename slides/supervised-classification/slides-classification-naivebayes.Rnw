...
NB is a generative technique multiclass technique. Remember: We use Bayes' theorem and only need $\pdfxyk$ to compute the posterior as:
$$\pikx = \postk = \frac{\P(\xv | y = k) \P(y = k)}{\P(\xv)} = \frac{\pdfxyk \pik}{\sumjg \pdfxyj} $$
...
\begin{vbframe}{NB: Categorical Features}
  ...
  We use a categorical distribution for $p(x_j | y = k)$ and estimate the probabilities $p_{kjm}$ that in class in $k$ $x_j = m$ by frequencies.

$$
% p(x_j | y = k) = \frac{(\sum x_i)!}{\prod_i x_i!} \prod_m p_{kjm}^{[x_j = m]}
p(x_j = m| y = k) = \prod_m p_{kjm}^{[x_j = m]}
$$
%
...
\begin{vbframe}{NB is linear in frequencies}
