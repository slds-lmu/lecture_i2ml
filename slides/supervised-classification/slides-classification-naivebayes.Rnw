...
The \emph{Naive Bayes classifier} is a generative technique for categorical response $y \in \gset$.\\
We make a \enquote{naive} \emph{conditional independence assumption}: the features given the category $y$ are conditionally independent of each other, so that we can simply write:
...
\begin{itemize}
  \item Covariance matrices can differ over classes but assumed to be diagonal.
...
Parameter estimation now has become simple, as we only have to estimate $\pdf(x_j | y = k)$, which is univariate (given the class k).
