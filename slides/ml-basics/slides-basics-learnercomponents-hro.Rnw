...
%! basics-supervised, basics-datatask, basics-learners, basics-riskminimization

%% Notes
%% See chapter 19 http://tug.ctan.org/macros/latex/contrib/beamer/doc/beameruserguide.pdf
...
\begin{itemize}
\item
  \textbf{Hypothesis Space:} What kind of model $f$ can be learned?\\
\item
  \textbf{Risk:} What is a well fitting model?\\
\item
  \textbf{Optimization:} How to search for the best model?\\
\end{itemize}


\note<1>{
Nearly all supervised learning algorithms can be described
in terms of three components: Hypothesis Space + Risk + Optimization.
...
\lz\lz\lz
\note{
Let's describe what a learner is!
This does not cover all special cases, but it's a useful framework for most supervised ML problems.
}
\end{frame}
