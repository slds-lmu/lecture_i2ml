...
So what could a learner look like? Let us consider a linear regression task with 
a single feature and a single target variable.

\begin{itemize}
  
  \item The \textbf{hypothesis space} in univariate linear regression is the set 
  of all linear functions, with $\thetab = (\theta_0, \theta)$:
  
  $$\Hspace = \{\fx = \theta_0 + \theta \xv: \theta_0, \theta \in \R \}$$
  
  \begin{center}
    \includegraphics[width = 0.4\textwidth]{figure_man/learner_lin_reg_placeholder.PNG} 
  \end{center}
  
  \item We might use the mean squared error as our loss function, punishing
  larger distances between observations and regression line more severely:
  
  $$\risket = \meanin (\yi - \theta_0 - \theta \xi)^2$$
  
  \begin{center}
    \includegraphics[width = 0.2\textwidth]{figure_man/learner_lin_reg_placeholder_3.PNG} 
  \end{center}
  ...
  \begin{center}
    \includegraphics[width = 0.4\textwidth]{figure_man/learner_lin_reg_placeholder_2.PNG} 
  \end{center}
