...
\newcommand{\titlefigure}{figure/hs-quadric-2.pdf}
...
\begin{itemize}
  ...
  \textcolor{red}{
  \item Therefore, we can rewrite the hypothesis space as: \\
  \lz
  $\Hspace = \{ f_{\thetab}:  f_{\thetab}$ belongs to a certain functional
  family, \\
  parameterized by $\thetab \}$
  }
  
  % \begin{equation*}
  %   \begin{skip}
  %     \Hspace = \{ f_{\thetab}:  &f_{\thetab} \text{ belongs to a certain 
  %     functional family \\
  %     &parameterized by } \thetab \}
  %   \end{skip}
  % \end{equation*}
