...
\begin{itemize}
\item We have seen, we can parmetrize models $f$ regarding a parametrization $\thetab \in \Theta$. 
...
\[
    \thetab^{[t+1]} = \thetab^{[t]} - \lambda \frac{\partial}{\partial\thetab}{\riske(\thetab^{[t]})}.
\]
...
\vspace{0pt}% 
So we choose a random starting point $\thetab^{[0]}$ with associated empirical risk $\riske(\thetab^{[0]}) = 76.25.$
\endminipage\hfill
\end{tabular}
...
\minipage{0.54\textwidth}
\vspace{0pt}% 
We arrive at $\thetab^{[1]}$ with associated empirical risk $\riske(\thetab^{[1]}) \approx 42.73.$ \\ Hence our estimation has improved since $\riske(\thetab^{[1]}) < \riske(\thetab^{[0]}).$
\endminipage\hfill
\end{tabular}
\end{vbframe}
\begin{vbframe}{Gradient Descent - Example}
...
\minipage{0.54\textwidth}
\vspace{0pt}% 
We arrive at $\thetab^{[2]}$ with associated empirical risk $\riske(\thetab^{[2]}) \approx 25.08.$ Thus clearly, we have improved our estimation again.
\endminipage\hfill
\end{tabular}
\end{vbframe}
\begin{vbframe}{Gradient Descent - Example}
...
\minipage{0.54\textwidth}
\vspace{0pt}% 
We iteratively improve our estimation by repeating this procedure until $\riske$ converges\footnote[frame]{This is not the only stopping criterium; e.g., we could limit the number of iterations.}. 
\endminipage\hfill
\\
\minipage{0.32\textwidth}
...
\minipage{0.54\textwidth}
\vspace{0pt}% 
Eventually we arrive at a minimum $\thetabh$ with $\riske(\thetabh) = 0.$
\endminipage\hfill
\end{tabular}
\end{vbframe}
...
\item Hence it weights components higher\footnote[frame]{in an absolute sense} in which $\riske$ decreases more. \\
...
\item Thus we use the learning rate $\lambda$ to scale the step length in each iteration. \\A $\lambda$ chosen too high can lead to poor convergence or no convergence at all. \\
A $\lambda$ chosen too low leads obviously to slow convergence.
\end{itemize}
\begin{figure}[!htb]
\minipage{0.32\textwidth}
  \includegraphics[width=\linewidth]{figure/grad_desc_lambda1}
  \caption*{\tiny good convergence for $\lambda1$}
\endminipage\hfill
\minipage{0.32\textwidth}
  \includegraphics[width=\linewidth]{figure/grad_desc_lambda2}
  \caption*{\tiny poor convergence for $\lambda_2 (> \lambda_1$)}
\endminipage\hfill
\minipage{0.32\textwidth}%
  \includegraphics[width=\linewidth]{figure/grad_desc_lambda3}
  \caption*{\tiny no convergence for $\lambda_3 (> \lambda_2$)}
\endminipage
\end{figure}
