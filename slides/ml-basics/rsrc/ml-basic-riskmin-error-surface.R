...
make_surface_plot <- function (filename = "figure/ml-basic-riskmin-error-surface.png", include_best = FALSE){
  # Create sample data
  set.seed(1234)
  ...
  jet.colors <- colorRampPalette( c("white", "grey") )
  ...
  pmat <- persp(x1seq, x2seq, z,
                ...
                ticktype="detailed", 
                #zlim = c(0, 3),
                xaxs = "i", # get rid of the margins 
                col = color[facetcol],#col = "lightblue",
                xlab = expression("θ[intercept]"), 
                ylab = expression("θ[slope]"),#expression(theta),#expression("θ[slope]"),
                zlab="R_emp")
  
  if (!include_best){
    # add example thetas as dots 
    mypoints = trans3d(theta_1, theta_2, r_emp, pmat = pmat)
    points(mypoints, pch = 19, col = "black", cex = 1.2)
    
    mypoints2 = trans3d(theta_1[4], theta_2[4], r_emp[4], pmat = pmat)
    points(mypoints2, pch = 19, col = "blue", cex = 1.2)
  }
  ...
  if (include_best){
    ...
    mypoints2 = trans3d(b0, b1, r_emp_best, pmat = pmat)
    points(mypoints2, pch = 19, col = "blue", cex = 1.2)
  }
...
make_surface_plot (filename = "figure/ml-basic-riskmin-error-surface.png", include_best = FALSE)
make_surface_plot(filename = "figure/ml-basic-riskmin-error-surface-best.png", include_best = TRUE)
