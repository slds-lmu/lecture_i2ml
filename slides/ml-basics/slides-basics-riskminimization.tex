...
\begin{vbframe}{Loss}
\begin{itemize}
  \item The \enquote{quality} of a prediction $\fx$ is measured by a \textbf{loss function} 
  \item $\Lxy$ that quantifies how \enquote{close} $\fx$ is to $y$. \\
  \item There are different possibilities to quantify the loss, e.g.:
  \begin{itemize}
    \item  $\Lxy = |\fx - y|$ or 
    \item  $\Lxy = (\fx - y)^2$.
    \end{itemize}
...
The ability of a model $f$ to reproduce the association between $\xv$ and $y$ that is
    present in the data $\D$ can be measured by the \textbf{average loss}: \\
    the empirical risk
    $$
  \riskef = \meanin \Lxyi.
  $$  
...
By calculating the risk models can be compared (here: absolute error):

\lz

\includegraphics[width=1\textwidth]{figure_man/ml-basic_riskmin-compare-risk.png}
\end{center}
...
To get the best model, this risk needs to minimized. 
\lz

The \textbf{learning} or training of a model is equal to find the model $f$ which has the smallest average loss:
