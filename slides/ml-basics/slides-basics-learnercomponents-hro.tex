...
\begin{itemize}
  ...
  \item As long as we didn't introduce penalization, you can just think of the risk as sum of the losses.
...
\end{itemize}
We can write the inducer as a formal mapping (where $\Lambda$ is a set of hyperparameters specific to the learner):
\[\inducer: \D \times \Lambda \rightarrow \Hspace\]
And as before, we can also adapt this concept to finding $\thetabh$ for parametric
models:
...
\begin{itemize}
  ...
  \end{center}
  \textbf{Design choice:} F.e. add more flexibility by allowing polynomial effects or by using a spline basis.
  ...
  \end{center}
  \textbf{Design choice:} F.e. use absolute error as loss function.
...
\end{figure}
  \textbf{Design choice:} F.e. use gradient descent or some other optimization procedure.
