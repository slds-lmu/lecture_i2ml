...
\item It allows us to see how fast a learner can learn the given relationship in the data.
...
\item It visualizes when a learner has learned as much as it can when
\begin{itemize}
