...
\begin{itemize}
  \item \textbf{Lin. Regr.}: $\Yspace = \R$, $\phi$ identity
  % $\rightarrow$ continous output
  \item \textbf{Log. Regr.}: $\Yspace = \{0, 1\}$, logistic sigmoid $\phi(\thx) = \frac{1}{1 + \exp(- \thx)} 
  =: \pixt$\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \texttt{LearnerClassifLiblineaR}, calling \texttt{LiblineaR::LiblineaR()} which uses coordinate descent for optimization
  \end{itemize}
