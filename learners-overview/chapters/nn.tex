...
\begin{itemize}
    ...
    \item Used as starting point (\textbf{pre-trained}) and fintuned via \textbf{transfer} or \textbf{few-shot} learning for other tasks requiring little data
...
\begin{itemize}
    ...
    \item Training pipeline (data-augmentation, training schedules, ...) is much crucial than the specific architecture
    \item While NNets are state-of-the-art for \textbf{computer vision (CV)} and \textbf{natural language processing (NLP)} do not use them for tabular data
...
\begin{itemize}
  \item \textbf{R:} Use python libraries (below) via \texttt{reticulate}, but not really recommendet except for toy applications.
