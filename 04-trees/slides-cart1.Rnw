...
\hspace{0.5cm}
\column{0.7\textwidth}
%source: https://docs.google.com/presentation/d/1Pns5UJyc-Aqyk9d-CLekA46BC7UJ_e0uSdErat47opM/edit#slide=id.p
...
$$I(\Np) = \sum_{k\neq k'} \pikN \pi^{(\Np)}_{k'} = \sum_{k=1}^g \pikN(1-\pikN)$$
