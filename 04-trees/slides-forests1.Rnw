...
\begin{vbframe}{Bagging}
  \begin{itemize}
    ...
    \item Typically decreases interpretability of the model.
  \end{itemize}
...
\begin{itemize}
  ...
  \item Let (x,y) be a random sample from $\P_{xy}$ that is independent of $\D$. The expected loss of the non-bagged $f(x)$ is then
    $e = \E_\D \E_{xy} [(y - f(x))^2]$, while it is
    $e_A = \E_{xy} [(y - f_A(x))^2]$ for the aggregated estimator.
 \item It follows:
  \begin{align*}
  e &= \E_{xy}\left[y^2 - 2 y f_A(x) + \E_{\D}\left[(f(x))^2\right]\right] \\
    &= \E_{xy}\left[y^2 - 2 y f_A(x) + \var_\D\left[f(x)\right] + (f_A(x))^2\right]\\
  e_A &= \E_{xy}\left[y^2 - 2 y f_A(x) + (f_A(x))^2\right]\\
      &= e - \E_{xy}\left[\var_\D\left[f(x)\right]\right]
  \end{align*}
 \item $\var_\D\left[f(x)\right]$ is also called the \enquote{ambiguity} of the ensemble and is a measure of the diversity of the baselearners. 
