...
\textbf{Aggregate} the predictions of the $B$ estimators:
  \begin{itemize}
    ...
    \item Posterior probabilities for $\xb$ in classification can be estimated by calculating class frequencies over the ensemble
  \end{itemize}

\begin{center}
% FIGURE SOURCE: https://docs.google.com/presentation/d/1lU1qPlY-NQkvWCDrrV6PdDYkloF3eFudM9v2NFYg1rY/edit#slide=id.g605d26c398_2_91
\includegraphics[width=0.8\textwidth]{figure_man/rf_agg_majvot.pdf}
\end{center}
...
\textbf{Example:}
\\
Let's say you have a binary classification task and your submodels are pretty bad ($ACC = 0.55$).
\\
If all models would be very similar to each other, then they would all classify the \textbf{same} observations right or wrong.
\\
If all submodels are very different, but still have an accuracy of $> 0.5$, then every new observation has a chance to be classified right by each sub-model with approximately this accuracy.
\\
Now, let's say you have 1000 observations and 500 completely different sub-models with an accuracy of $ACC = 0.55$ each. To be classified right by the ensemble an observation has to be classified right by \textbf{more than 250} of these sub-models. For each observation we draw one binomial random number with $n = 500$ trials and $p = 0.55$ chance of success.

\framebreak

Histogram of $X_i \sim B(500, 0.55)$, for $i = 1, ..., 1000$.
...
<<echo = FALSE, eval = TRUE, fig.height = 4>>=

df <- data.frame(x = rbinom(1000, 500, 0.55))
df$classified <- ifelse(df$x > 250, "right", "wrong")
# head(x)
# sum(x > 250)

ggplot(df, aes(x = x, fill = classified)) +
  geom_histogram(binwidth = 3) +
  scale_f_d() +
  geom_vline(xintercept = 250.5) +
  xlab("Number of models with right classification") +
  ylab("Number of observations")

@

All the observations on the right side of the vertical line are classified right by more than 250 of the submodels and with this are classified right by the whole ensemble.
...
\begin{center}
%FIGURE SOURCE: https://docs.google.com/presentation/d/1lU1qPlY-NQkvWCDrrV6PdDYkloF3eFudM9v2NFYg1rY/edit#slide=id.g605d26c398_2_91
\includegraphics[width=0.7\textwidth]{figure_man/rf_visualization.pdf}
\end{center}
\end{vbframe}
...
\begin{itemize}
  ...
  \item Predict all $\xb$ with trees that didn't see it, average error
