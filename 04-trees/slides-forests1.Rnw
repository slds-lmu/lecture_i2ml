...
\\
If all submodels are very different, but still have an accuracy-rate of $> 0.5$, then every new observation has a chance to be classified right by each sub-model with approximately this accuracy-rate.
\\
Now, let's say you have 1000 observations and 500 completely different sub-models with an accuracy-rate of $acc = 0.55$ each:
