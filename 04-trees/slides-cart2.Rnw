...
p1 = ggplot(data = plot.data, aes(x = Category, y = Frequency, fill = Category)) +
  ...
  ggtitle("1)") + ylab("Frequency of class 1") + xlab("Category of feature") + theme_bw()
...
p1 = ggplot(data = plot.data, aes(x = Category, y = Mean, fill = Category)) +
  geom_bar(stat = "identity")  + theme(legend.position = "none") +
  ggtitle("1)") + ylab("Mean of outcome") + xlab("Category of feature") + theme_bw()
...
\item During tree growing, it is hard to tell when we should stop, as we do not know if the addition
...
pl = plotLearnerPrediction(rpart, task, gridsize = 300, cv = 0, err.mark = "none",
  prob.alpha = FALSE)
pl = pl + geom_abline(slope = 1, linetype = 2) + theme_bw()
print(pl)
@
...
pl = pl + theme(legend.position = "none") + theme_bw()
print(pl)
@
Prediction function is not smooth because a step function is fitted.
\end{vbframe}
