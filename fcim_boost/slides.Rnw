...
\begin{itemize}
\item \emph{Weak learners} are defined as a prediction rule with a correct classification rate that is at least slightly better than random guessing (> 50\% accuracy).
\item We call a learner a \emph{strong learner} \enquote{if there exists a polynomial-time algorithm that achieves low error with high confidence for all concepts in the class} (Schapire 1990)
...
\begin{itemize}
  \item The \emph{AdaBoost} (Adaptive Boosting) algorithm is a \emph{boosting} method
...
\begin{itemize}
  ...
  \item Base learner models $\bmm$ are binary classifiers that map to $\Yspace = \{-1,+1\}$.
    We might sometimes write $b(x, \thetam)$, instead.
  \item Predictions from all base models $\bmm$ are combined to form:
    $$
    \fx = \sum_{m=1}^{M} \betam \bmm(x)
    $$
  ...
  \item The discrete prediction function is $h(x) = \text{sign}(\fx) \in \{-1,1\}$.
\end{itemize}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \EndFor
    \State Output: $\fxh = \sum_{m=1}^{M} \betamh \bmmh(x)$
  \end{algorithmic}
\end{algorithm}
...
then we fit a regression model $b(x, \thetam)$ to them:
...
\begin{enumerate}
  ...
  \item
    To improve the model we calculate the pointwise residuals on the training data $\yi - f(\xi)$ and fit a GAM fitted on the residuals.
...
One can write a tree as: $ b(x) = \sum_{t=1}^{T} c_t \I(x \in R_t) $,
...
\begin{align*}
  \fm(x) &= \fmd(x) +  \betam \bmm(x) \\
         &= \fmd(x) +  \betam \sum_{t=1}^{\Tm} \ctm \I(x \in \Rtm)
\end{align*}
...
$$ \fm(x) = \fmd(x) +  \sum_{j=1}^{\Tm} \ctmt \I(x \in \Rtm) $$
...
$$ \pikx = s_k(f_1(x), \ldots, f_g(x)) = \exp(\fxk) / \sum_{j=1}^g \exp(f_j(x)) $$

Multinomial loss $L$:
$$ L(y, f_1(x), \ldots f_g(x)) = - \sumkg \I(y = k) \ln \pikx $$

And for the derivative the following holds:
$$-\fp{L(y_k, f_1(y), \ldots, f_g(x))}{\fxk} =  \I(y = k) - \pikx $$
...
\begin{itemize}
  ...
  $$ \hat{c}_{tk}^{[m]} = \argmin_{c} \sum_{i=1}^n \sum_{k=1}^g L \left( \yi_k, f^{[m-1]}(\mathbf{x}^{(i)}) + \sum_{t=1}^T \hat{c}_{tk} \I\left(x_i \in R_{t}^{[m]}\right) \right)$$

  where $L$ is the multinomial loss function $L(y, f_1(x), \ldots f_g(x)) = - \sumkg \I(y = k) \ln \pikx$
  and $\pikx = \frac{\exp(f_k(x))}{\sum_j \exp(f_j(x))}$ as before.
