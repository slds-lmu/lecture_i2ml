...
ggTrainTestPlot(data = mydf, truth.fun = .h, truth.min = 0, truth.max = 1, test.plot = FALSE,
  test.ind = ind)[["plot"]] + ylim(0, 1) + theme_bw()
@
...
\item That implies that for a given dataset $\D$, we have to preserve some data for testing that we cannot use for training
\end{itemize}
\end{vbframe}
...
ggTrainTestPlot(data = mydf, truth.fun = .h, truth.min = 0, truth.max = 1, test.plot = TRUE,
  test.ind = ind, degree = c(1, 3, 9))[["plot"]] + ylim(0, 1) + theme_bw()
@
...
pl = pl + facet_wrap(vars(task), scales = "free") + theme_bw()
print(pl)
@
\end{vbframe}
...
pl1 = pl1 + theme(axis.text.x = element_text(angle = 45)) + theme_bw()
print(pl1)
@
...
pl2 = pl2 + theme(axis.text.x = element_text(angle = 45)) + theme_bw()
print(pl2)
@
