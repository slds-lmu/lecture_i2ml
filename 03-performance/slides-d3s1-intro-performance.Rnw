...
learner = makeLearner(cl = "classif.knn", k = 3)
...
qda.data.test$pred.true = ifelse(pred.test$data$truth != pred.test$data$response, "incorrect", "correct")
qda.data.train$pred.true = ifelse(pred.train$data$truth != pred.train$data$response, "incorrect", "correct")
...
trainplot = ggplot(data = qda.data.train) +
  ...
  ggtitle(label = paste0("Prediction on train data with mmce = ", as.numeric(performance(pred.train))) )
#trainplot
testplot = ggplot(data = qda.data.test) +
  ...
  scale_y_continuous(limits = c(-5, 5)) + 
theme(legend.justification = c(1, 0), legend.position = c(1, 0))+
  ggtitle(label = paste0("Prediction on test data with mmce = ", as.numeric(performance(pred.test))) )
#testplot
...
\lz Examples:
In logistic regression we minimize the binomial loss and in k-NN there is no
...
Fit two models with polynomial degree 1 and 9 on 30 bootstrapped train data samples
...
# bs samples
df.train = cbind(x, y)
df.all = cbind(x.all, h(x.all))
seedlist = seq(190, 220, 1)
...
for (i in 1:length(seedlist)) {
  ...
  p1 = lm(y ~ poly(x, 1, raw = TRUE), data = df.bs)

  x.plot = seq(0, 1, length = 500L)


  lines(x.plot, predict(p1, newdata = data.frame(x = x.plot)),
    col = "#E69F00AA", lwd = 2L)
}
...
for (i in 1:length(seedlist)) {
  ...
  p9 = lm(y ~ poly(x, 9, raw = TRUE), data = df.bs)

  x.plot = seq(0, 1, length = 500L)

  lines(x.plot, predict(p9, newdata = data.frame(x = x.plot)),
    col = "#56B4E9AA", lwd = 2L)
}
...
Example 1: we want to train a model to fit the true generating process of this data:
