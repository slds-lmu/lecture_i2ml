...
@
\lecturechapter{Evaluation: ROC Analysis}
...
\begin{vbframe}{Binary Classifiers and Imbalanced Labels}
\begin{itemize}
...
\begin{itemize}
  ...
  \item In our example, classifying a sick patient as healthy, should incurs a much higher loss then classifying a healthy patient as sick
  \item The costs depend a lot on what happens next: We can likely assume that our system is some type of screening filter, 
    often the next step after labeling someone as "sick" might be a more invasive, expensive, but reliable test for the disease
  ...
  \item We could also see this as imbalanced costs of misclassification, rather than imbalanced labels; both situations are tightly connected
\end{itemize}
\end{vbframe}
...
\begin{itemize}
  ...
  \item Unfortunately, users often have a notoriously hard time to come up with precise numbers in these scenarios
...
For a binary problem / predictor we can normalize in rows and cols of the 
confusion matrix to derive several informative metrics in imbalanced 
or cost-sensitive situations:

\includegraphics[width=0.8\textwidth, height=4cm]{figure_man/confusion_matrix.png}
...
\begin{center}
  \includegraphics[width=\textwidth, height=7cm]{figure_man/confusion_matrix_example.png}
\end{center}
...
It is difficult to achieve a high \textit{Positive Predictive Value} and high \textit{True Positive Rate} simultaneously.
A trade-off offers the $F_1$-measure, which is the harmonic mean of PPV and TPR:

$$F_1 = 2 \cfrac{PPV \cdot TPR}{PPV + TPR} = \cfrac{2\text{TPR}}{\text{TPR} + \tfrac{neg}{pos} \cdot \text{FPR} + 1 } $$
...
\begin{itemize}
  \item A model with $TPR=0$ (no one predicted as postitive from the positive class) or $PPV=0$ (no real positives among the predicted) has an F1 of 0
  \item Predicting every one as N has F1=0  
  \item Predicting every one as P has F1=0  
...
\begin{itemize}
  \item Plot TPR vs FPR; False Positive Rate: $FPR = 1 - TNR$
  \item The best classifier lies on the top-left corner
  \item Diagonal contains worst possible solutions
\end{itemize}
\begin{center}
\includegraphics[width=0.7\textwidth, height=5.5cm]{figure_man/roc-space.png}
\end{center}
\end{vbframe}

\begin{vbframe}{ROC Analysis}
ROC curves are insensitive to class distributions (at prediction time).
\begin{table}[]
\centering
\begin{tabular}{|l|c|c|}
                \hline
               & Class Positive & Class Negative \\ \hline
Pred. Positive & 40            & 25            \\ \hline
Pred. Negative & 10            & 25           \\ \hline
\end{tabular}
\end{table}

Here we have we have a proportion $\text{pos}/\text{neg} = 1$. MCE = 35/100.
\lz
Now we change the proportion to $\text{pos}/\text{neg} = 2$. MCE = 30/100.
...
The True Positive Rate and ($\text{TPR} = 0.8$) and the False Positive Rate ($\text{FPR} = 0.5$) do not change.
...
\begin{vbframe}{Confusing alternative terminology}
...
\begin{tabular}{l c c}
\hline
measure &  synonyms & formula \\
\hline
True Positive Rate (TPR) & recall, sensitivity, hit rate & $\frac{\#TP}{\#TP + \#FN}$  \\
True Negative Rate (TNR) & specificity, selectivity & $\frac{\#TN}{\#FP + \#TN}$  \\
...
false positive rate (FPR) &  fallout, false alarm rate & $1 - TNR$ \\
false discovery rate & & $1 - PPV$ \\
false omission rate & & $1 - PPV$ \\
\hline
\end{tabular}
}
\end{table}
