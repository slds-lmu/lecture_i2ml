...
\begin{vbframe}{Multiclass Confusion Matrix}
  ...
  \begin{itemize}
    ...
    \item Example (Positive Predictive Value for label A):
    $$\text{PPV}_A = \cfrac{\# \text{TP}_A}{\# \text{TP}_A + \# \text{FP}_A} = \cfrac{30}{30+20+10} = 0.5$$
  \end{itemize}
...
\begin{itemize}
  ...
  \item   False Positive Rate, $\text{FPR} = 1-\text{TNR} = \cfrac{\text{\#FP}}{\text{\#FP} + \text{\#TN}}$
\end{itemize}
