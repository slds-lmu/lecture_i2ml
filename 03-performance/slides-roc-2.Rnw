...
pl = pl + geom_text_repel(data = data.frame(FPR = 0.20, TPR = 0.80, label = "unclear winner"))
...
\begin{vbframe}{ROC Space}
  ...
  \item The diagonal is worst, where classifiers produce random labels (with different proportions). If each positive $x$ will be randomly classified with 20\% as "pos", $TPR = 0.25$. If we assign each negative $x$ randomly to "pos", $FPR = 0.25$.
\end{itemize}
<<echo=FALSE, out.height="5cm", fig.align="center">>=
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item If $x$ is "neg", move FPR $1/n_i$ right, as we have one FP more
  \end{itemize}
\end{itemize}
\end{vbframe}
