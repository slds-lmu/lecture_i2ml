...
\end{itemize}
<<echo=FALSE, out.height = "0.6\\textheight", out.width = "0.6\\textwidth", height = 3, fig.align="center">>=
...
pl = pl + geom_text_repel(data = data.frame(FPR = 0.25, TPR = 0.65, label = "dominates")) + theme_bw()
print(pl)
@
\end{vbframe}
...
\begin{vbframe}{ROC Space}
  ...
  \item The diagonal is worst, where classifiers produce random labels (with different proportions). If each positive $x$ will be randomly classified with 25\% as "pos", $TPR = 0.25$. If we assign each negative $\xb$ randomly to "pos", $FPR = 0.25$.
\end{itemize}

\vspace*{-0.1cm}

<<echo=FALSE, out.height = "0.55\\textheight", out.width = "0.55\\textwidth", height = 6, fig.align="center">>=
fpr = c(0, 1, 0, 0.25, 0.75)
tpr = c(1, 1, 0, 0.25, 0.75)
label = c("Best", "Pos-100%", "Pos-0%", "Pos-25%", "Pos-75%")
pl = plot_roc_space(fpr, tpr, label) + theme_bw() 
print(pl)
@
\end{vbframe}
...
label = c("C1", "C2")
pl = plot_roc_space(fpr, tpr, label)
pl = pl + geom_segment(x = fpr[1], y = tpr[1], xend = fpr[2], yend = tpr[2], lty = "dotted") + theme_bw()
print(pl)
@
\end{vbframe}
...
\end{itemize}
\begin{centering}
% FIGURE SOURCE: https://drive.google.com/open?id=1GmlgtjSCTHgSAveVGf-x1ojAjGP2llPhFKjn_6M4Sig
...
plot_roc_curves = function(d) {
  ...
  pl = pl + xlim(c(0, 1)) + ylim(c(0,1))
  pl = pl + geom_abline(slope = 1, lty = "dotted") + theme_bw()
}
n = 10
...
\begin{center}
% FIGURE SOURCE: https://drive.google.com/open?id=1xj9_84181bqFpr0EMqdGHE6dUf_vAf1qcs9z-siUsCw
