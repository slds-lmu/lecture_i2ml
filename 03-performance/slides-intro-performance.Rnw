...
\vfill
\textit{"The generalization performance of a learning method relates to its prediction capability on independent test data.
Assessment of this performance is extremely important in practice, since it guides the choice of learning
method or model, and gives us a measure of the quality of the ultimately chosen model."}

\hfill Hastie et al. - The Elements of Statistical Learning
\vfill
...
%\lz
\begin{itemize}
\item \textbf{Inner Loss} which is optimized during model fitting to obtain $\fh$
\item \textbf{Outer Loss} which will be used to assess the model afterwards
\end{itemize}
...
\begin{centering}
\includegraphics[width=9cm,page=1]{figure_man/make_classif_regr_error_intro_plot}
\end{centering}

To compare the performance of two ML models on a single observation or a new data set,
we would ideally like to boil them down to a \textbf{single} numeric measure of the performance.
...
Depending on the modeling problem, different \textbf{performance measures} are required:
...
\hline
 ...
 Recall 	        &  Mean Absolute Error 	    \\
 Precision 	    &  Median Squared Error	    \\
 F1-Score 	      &  Relative Absolute Error	\\
 AUC	            &  R-squared	              \\
 Brier Score     &  Adj. R-squared           \\
 ...     &  ...           \\
\hline
\end{tabular}
\end{center}
...
\begin{centering}
\includegraphics[width=10cm,page=1]{figure_man/loss_regression}
\end{centering}
...
The absolute error is more robust. On the other hand, the absolute error is not differentiable
at $0$.
...
Another often used measure is $R^2$. It measures the \textit{fraction of variance explained} by the model.
...
\begin{itemize}
  \item If $R^2$ is measured on the training data: $R^2 \in [0;1]$.
  \item If $R^2$ is measured on held out data: $R^2 \in [-\infty;1]$.
\end{itemize}
...
\begin{vbframe}{Performance Measures - Classification}

The Confusion matrix is an intuitive metrics used for establishing the correctness and accuracy of a model.
It is used for classification problems where the output can be of two or more types of classes.

\begin{centering}
\includegraphics[width=8cm,page=3]{figure_man/confusion_matrix_measures}
\end{centering}

\end{vbframe}

\begin{vbframe}{Classification: Accuracy / Classification Error}
...
\textbf{Accuracy; Classification Error = 1 - Accuracy}

\begin{centering}
\includegraphics[width=8cm,page=4]{figure_man/confusion_matrix_measures}
\end{centering}
...
\begin{vbframe}{Classification: Balanced Accuracy / Error}

\textbf{Problem:} Accuracy is sensitive for imbalanced data situations.
\textbf{Solution:} Compute the accuracy for each class label

\begin{centering}
\includegraphics[width=8cm,page=5]{figure_man/confusion_matrix_measures}
\end{centering}

\end{vbframe}

\begin{vbframe}{Classification: Brier Score}

Measures squared distances of probabilities from the true class labels:

\[
BS ={\frac {1}{n}} \sumin \left(\pi^{(i)}(x) - \yi\right)^2
\]

\begin{itemize}
  \item Above is the usual definition for binary case, $\yi$ must be coded as 0 and 1.
  \item Measures how well calibrated probabilities are.
\end{itemize}

\end{vbframe}

\begin{vbframe}{Threshold}

Thresholding flexibly converts measured probabilities to labels.

\begin{centering}
\includegraphics[width=10cm,page=2]{figure_man/confusion_matrix_measures}
\end{centering}
