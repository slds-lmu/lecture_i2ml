...
\begin{vbframe}{Bias-Variance Analysis for Subsampling}
  ...
  \begin{itemize}
    \item Let's look at the performance evaluation of a classification task.
    \item We use split-rates $s \in \{0.05, 0.1, ..., 0.95\}$ for training with $|\Dtrain| = s \cdot 500$.
    \item We compare 50 subsampling experiments with $50 \cdot 50$ hold-out experiments per split.
