...
\begin{itemize}
  \item Supervised machine learning deals with \textit{labeled} data, i.e., we have input data $x$ and the outcome $y$ of past events.
...
\begin{itemize}
  \item It puts all variables into \textit{comparable} units, i.e., we make sure that all normalized variables have mean 0 and standard deviation of 1.
  \item It can avoid numerical instabilites in several algorithms, e.g. if a variable has very low / high values.
  \item It helps in computing meaningful \textit{distances} between observations.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Fix the first PC and proceed with rotating the remaining $p-1$ coordinates until the second PC (which is orthogonal to the first PC) is found that explains most of the \textit{remaining} variation, etc.
...
\begin{frame}{Example: The Olympic Heptathlon Data}

The \enquote{heptathlon} data set in the R package \enquote{HSAUR3} contains the competition results of 25 athletes in 7 disciplines for the Olympics held in Seoul in 1988.

<<message=FALSE, out.width="0.5\\textwidth", eval = 2>>=
...
Variables of the \enquote{heptathlon} data:


\begin{itemize}

\item \enquote{hurdles}: results 100m hurdles (in seconds).
\item \enquote{highjump}: results high jump (in m).
\item \enquote{shot}: results shot putt (in m).
\item \enquote{run200m}: results 200m race (in seconds).
\item \enquote{longjump}: results long jump (in m).
\item \enquote{javelin}: results javelin (in m).
\item \enquote{run800m}: results 800m race (in seconds).
\item \enquote{score}: total score of the official scoring system.
...
The variables \enquote{hurdles}, \enquote{run200m} and \enquote{run800m} are time measurements, i.e. low values are better.
...
\begin{frame}{Scatter Plot Matrix}
...
We can compute all pairwise correlations of the variables (without the \enquote{score} column):

% PROBLEM HERE!
<<echo=FALSE, out.width = "0.95\\textwidth">>=
...
<<message=FALSE, fig.align="center", fig.height = 5, out.width = "0.75\\textwidth", echo = -c(1, 3)>>=
...
\begin{frame}{Perform PCA using \enquote{princomp()}}
...
\begin{itemize}
  \item In R, the `princomp()` function carries out a PCA via an eigendecomposition of the sample covariance matrix $\Sigma$.
...
The resulting object of \enquote{princomp()} contains at least:
...
<<message=FALSE, fig.align="center", fig.height = 5, out.width = "0.6\\textwidth">>=
...
<<fig.align="center", fig.height = 5, out.width = "0.5\\textwidth">>=
...
\begin{frame}{Proportion of Explained Variance}
...
\begin{frame}{Choosing the Number of PCs}
...
\item Use a scree plot: Plot the component variances vs. the component number and look for an \textit{elbow}.
...
<<fig.align="center", fig.height = 4, out.width = "0.7\\textwidth", echo = FALSE>>=
...
\begin{frame}{PC Scores vs. Official Scores}
...
<<fig.align="center", fig.width = 8, fig.height = 6, out.width= "0.6\\textwidth", echo = FALSE>>=
