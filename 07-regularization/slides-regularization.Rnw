...
We are looking for models that 
    \begin{enumerate}
      \item fit well (low empirical risk $\riskef$),
      \item but not be too complex (low $J(f)$). 
    \end{enumerate}

\lz 

We decide to combine the two in a weighted sum called \textbf{regularized risk} and to control the trade-off via the complexity control parameter $\lambda$.

\begin{eqnarray*}
\riskrf &=& \riskef + \lambda \cdot J(f) \\
        &=& \sumin \Lxyi + \lambda \cdot J(f)
\end{eqnarray*}

\vfill

\begin{footnotesize}
\textbf{Note:} As for $\riske$, often $\riskr$ and $J$ are defined on $\thetab$ instead of $f$, so $\riskrt = \risket + \lambda \cdot J(\thetab)$. 
\end{footnotesize}
...
\begin{itemize}
  ...
  \item $\lambda$ is called \textbf{complexity control parameter}. 
...
\textbf{Regularization} is a major tool to handle overfitting. 

  \begin{itemize}
    \item  In the broadest sense, regularization refers to \textbf{any} modification made to a learning algorithm that is intended to reduce its generalization error but not its training error.
    \item Regularized empirical risk minimization is \textbf{one} common way to perform regularization$^{(*)}$ 
    \item  Explicitly or implicitly, such modifications represent the preferences we have regarding the elements of the hypothesis set, for example the preference of having models with less features. 
  \end{itemize}

  \vfill

  \begin{footnotesize}
  $^{(*)}$ There are other forms of regularization, e.g. by performing less aggressive risk minimization through early stopping iterative optimization algorithms. 
  \end{footnotesize}
...
For the (unregularized) linear model we optimize 
$$
...
\begin{vbframe}{Ridge Regression}
  ...
  $\thetab$ towards $0$, by putting an $L_2$-penalty ($J(\thetab) = \|\thetab\|^2 = \sqrt{\theta_1^2 + \theta_2^2 + ... + \theta_p^2}$) on it:
  \begin{eqnarray*}  
...
% $$
% \nabla g_\lambda(\thetah_{Ridge}) = - 2 \bm{y}^T\bm{X} + 2 (\thetah_{Ridge})^\top \bm{X}^T\bm{X} + 2 \lambda (\thetah_{Ridge})^\top = 0.
% $$
...
% $$
% \nabla_\theta L(\bm{\theta}, \alpha)= - 2\bm{y}^T\bm{X} + 2 \bm{\theta}^T \bm{X}^T\bm{X} + 2 \alpha \bm{\theta}^T = 0.
% $$
...
Another shrinkage method is the so-called \textbf{Lasso regression}, which uses a $L_1$ penalty on $\thetab$ ($J(\thetab) = \|\thetab\|_1 = |\theta_1| + |\theta_2| + ... + |\theta_p|$$):
...
$$
\mathcal{R}_{\text{elnet}}(\thetab) =  \sumin (\yi - \thetab^T \xi)^2 + \lambda_1 ||\thetab||_1 + \lambda_2 \|\thetab\|_2^2.
$$
