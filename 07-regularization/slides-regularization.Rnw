...
\begin{vbframe}{Ridge Regression}
  ...
  \begin{eqnarray*}  
  \thetah_{\text{Ridge}} &=& \argmin_{\thetab} \sumin \left(\yi - \thetab^\top \xb \right)^2 + \lambda ||\thetab||_2^2 \\
...
\framebreak 
  ...
  \begin{eqnarray*}
     \min_{\thetab} && \sumin \left(\yi - \thetab^\top\xb\right)^2 \\
...
pl1 = plot_coef_paths(path_l1$path, featnames, "lambda") + theme_bw() + ggtitle("Lasso")
pl2 = plot_coef_paths(path_l2$path, featnames, "lambda") + theme_bw() + ggtitle("Ridge")
pl3 = plot_cv_path(path_l1$cv_lam, "lambda") + theme_bw() 
pl4 = plot_cv_path(path_l2$cv_lam, "lambda") + theme_bw() 
