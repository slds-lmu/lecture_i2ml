...
$$
\fxt = \thetab^T\xb = \theta_0 + \theta_1 x_1 + \theta_2 x_2 + ... + \theta_{12} x_{12}
$$
...
\begin{itemize}
\item \textbf{maximizing the fit} (minimize the train loss) $\min_f \frac{1}{n}\sumin \Lxyi$
...
\begin{vbframe}{Regularized Empirical Risk Minimization}
  ...
  Explicitly or implicitly, such modifications represent the preferences we have regarding the elements of the hypothesis set, for example the preference of having models with less features
...
\begin{itemize}
  \item  Coefficient estimates for ordinary least squares with $\fxt = \thetab^\top \xb$ usually require a full rank design matrix.
  ...
  \item One can deal with this issue by adding a complexity penalty to the loss:
  $$
  \riskrt = \sumin \left(\yi - \fxit\right)^2 + \lambda \cdot J(\thetab). 
  $$
  \item In the following we will present two possible choices for $J(\thetab)$.
\end{itemize}
...
\begin{vbframe}{Ridge Regression}
  ...
  \begin{eqnarray*}  
  \thetah_{\text{Ridge}} &=& \argmin_{\thetab} \sumin \left(\yi - \fxit \right)^2 + \lambda ||\thetab||_2^2 \\
...
We can, of course, also construct $L_1$ or $L_2$ penalized logistic regression to enable coefficient shrinkage and variable selection in this model. 
