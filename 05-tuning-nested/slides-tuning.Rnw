...
\item We estimate the generalization performance by evaluating the model on the test set $\Dtest$: $\GEh{\fh} = \frac{1}{|\Dtest|} \sum\limits_{(\xb, y) \in \Dtest} L\left(y, \fxh \right)$
\end{itemize}
...
\begin{figure}
  \includegraphics[width = 0.9\textwidth]{figure_man/automl1.png}
\end{figure}
...
\begin{eqnarray*}  
\min_{\lambda \in \Lambda} && \GEh~\left(\hat f\right) \\
s.t. && \hat f = \mathcal{I}(\Dtrain, \lambda)
\end{eqnarray*}
...
\item It internally performs empirical risk minimization, and returns the optimal model $\hat f$. 
\end{itemize}
...
\begin{itemize}
  ...
  \item One approach is \textbf{successful halving}: Given an initial set of configurations, all trained for a small initial budget, repeat:
  \begin{itemize}
