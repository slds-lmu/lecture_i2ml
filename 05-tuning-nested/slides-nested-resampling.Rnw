...
\begin{center}
\includegraphics[width=0.8\textwidth]{figure_man/autotune_in_model_fit.jpg}
\end{center}
\end{vbframe}

  
\begin{vbframe}{Train, Validation and Test Set}
...
\begin{footnotesize}
Assume we want to tune over a set of candidate hyperparameter configurations $\lambda_i; i = 1, \dots$ with 4-fold CV in the inner resampling and 3-fold CV in the outer loop. The outer loop is visualized as the light green and dark green parts. 
\end{footnotesize}
