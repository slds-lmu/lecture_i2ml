...
\begin{vbframe}{The Data Science Workflow}
  ...
  \item Train the model(s)
   \item Evaluate model performance on independent test data which was not used in any way for training (not even: data-dependent preprocessing).
...
Additional criteria:
  ...
  \item Training data size: e.g. simple regression or kNN can be problematic if more features than observations in the training data.  
