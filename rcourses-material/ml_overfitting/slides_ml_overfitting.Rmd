...
Overfitting learner \\
\vspace{0.5cm}
```{r, echo=FALSE, results='hide', out.width='\\textwidth'}
library(mlr)
library(mlbench)
...
plotLearnerPrediction(learner, task, P = TRUE, E = TRUE, pointsize = 4)
```
...
\vspace{0.5cm}
```{r, echo=FALSE, results='hide', out.width='\\textwidth'}
plotLearnerPrediction(learner, task, pointsize = 4)
```
...
-   $\theta_j$ are the coefficients and $d$ is called the degree (or order).
-   For a fixed $d$, the form of the function $\fx$ is determined by the values of the coefficients $\theta_j$.
-   Therefore, the model function has the form $f(x)$, and the task is to find the $\theta$ that best fits the data.

## Example: Polynomial Regression

Consider the following true relationship $f(x)$ with the corresponding training and test set created by the *holdout* method:

```{r, echo=FALSE, out.width="0.9\\textwidth", fig.height=5}
...
Models of different *complexity*, i.e. of different polynomial order $d$, are fitted to the training set:
...
-   The polynomial of order $d = 9$ was almost optimal on $\Dtrain$ but lacked in performing good on new data points $\Dtest$ (leads to overfitting as the model is too complex / flexible).
-   The linear model with $d = 1$ was not able to capture the non-linear relationship, even on $\Dtrain$ (leads to underfitting as the model is not complex enough).
 -->
...
-   Randomly divide the data into:
    - *Training set* $\Dtrain$ which you give to the learner
    - *Test set* $\Dtest$ which you hide to verify predictive performance -->
