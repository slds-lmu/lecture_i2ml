...
An expert in the field, researcher Son challenges his colleagues to explain the concept of bagging in one sentence, without using the following words: bootstrap, sample, aggregate, ensemble, base learner.
...
However, researcher Son notes that there is more to random forests than just merely applying bagging to trees.
...
\begin{enumerate}\bfseries
  \item[3)] Explain each measure in your own words, using as few sentences as possible. Use the pseudocode describing the algorithms to calculate the measures in the lectures slides. 
\end{enumerate}