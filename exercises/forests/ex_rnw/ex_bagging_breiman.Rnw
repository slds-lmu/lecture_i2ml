...
For a fixed observation $(\xv, y)$, show that the quadratic loss of the ensemble prediction $\fM$ is less than or equal to the average loss over individual base learner predictions $\blx$.
