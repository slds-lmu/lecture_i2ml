...
\begin{itemize}
%	
	\item If $ \P(y = 0~|~ \xv = \xv) \geq  \P(y = 1~|~ \xv = \xv) $ (or  $\P(y = 0~|~ \xv = \xv)\geq 1/2$), then any $\pix$ such that $\pix \geq 1/2$ minimizes $\E_{y|x} [ L(y, \pi(\xv)) ~|~ \xv = \xv ] .$
	
%	
	\item If $ \P(y = 0~|~ \xv = \xv) \leq  \P(y = 1~|~ \xv = \xv) $ (or  $\P(y = 0~|~ \xv = \xv)\leq 1/2$), then any $\pix$ such that $\pix < 1/2$ minimizes $\E_{y|x} [ L(y, \pi(\xv)) ~|~ \xv = \xv ] .$
%	
\end{itemize}
%
...
%	
	= \begin{cases}
			\geq 1/2 , & \mbox{if $\P(y = 0~|~ \xv = \xv)\geq 1/2,$} \\
			< 1/2 ,    & \mbox{if $\P(y = 0~|~ \xv = \xv)< 1/2,$} 
	\end{cases}
%	
\end{align}
%
...
%
The posterior distribution $p_{y|x}(1 ~|~ \bm{x})$ is quite naturally of this form, but it is in general not the only $\pi$ of this kind.
%
...
%
Thus, the corresponding loss function should give an incentive for any learning algorithm to do this by minimizing the loss function.
%
