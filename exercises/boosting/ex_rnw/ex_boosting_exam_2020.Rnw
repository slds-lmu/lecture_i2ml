...
\item[(ii)] the pseudo residuals $\bm{r}^{[1]}$ (use $L_2$-loss if you haven't been able to solve (a)),
...
\end{itemize}
for tree base learner with depth $d=1$ (stumps) and a step length $\nu = 1$. You are allowed to use results from the lecture. If you have not managed to derive the pseudo-residuals for the $\log_2$-normal distribution, use an $L_2$-loss.
...
\item[(ii)] $n$ grows
\end{itemize}
in terms of the model capacity (if $d$ is kept fixed)?
\end{enumerate}
