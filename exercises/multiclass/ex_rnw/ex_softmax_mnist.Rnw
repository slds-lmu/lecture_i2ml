\begin{enumerate}
\item Read in the MNIST data set inlcuded in the data sets in the package \texttt{keras}, using either 
<<eval = FALSE>>=
library(keras)
mnist <- dataset_mnist()
@
...
<<echo = FALSE, fig.height=2, fig.width=5, fig.align='center'>>=
library(keras)
mnist <- dataset_mnist()
...
@
\item Convert the features to a (pandas) data frame, by flattening the 28x28 images to a 784 entry long vector, which represents one row in your data frame. Divide the intesity values of each pixel (each column) by 255 to get a value between 0 and 1.  
...
\item Use the deep learning API keras to fit a softmax regression by fitting a network like the following:
