...
In logistic regression (binary case), we estimate the probability $p(y = 1 | \xv, \thetab) = \pi(\xv | \thetab)$. In order to decide about
the class of an observation, we set $\hat{y} = 1$ iff $\pi(\xv | \thetab) \geq \alpha\) for some \(\alpha \in (0, 1)$.
