...
$$\hat{\pi}_\text{yes}(x) = \frac{\hat{\alpha}_\text{yes}(x)}{\hat{\alpha}_\text{yes}(x) + \hat{\alpha}_\text{no}(x)} \approx 0.41.$$
    ...
    df_banana <- data.frame(
      Color = as.factor(c("yellow", "yellow", "yellow", "brown", "brown", "green", "green", "red")),
      Form = as.factor(c("oblong", "round", "oblong", "oblong", "round", "round", "oblong", "round")),
      Origin = as.factor(c("imported", "domestic", "imported", "imported", "domestic", "imported",
        "domestic", "imported")),
