...
\begin{align}
\LL & = \P(Y=y ~|~ \xv) = \pi(\xv)^y (1-\pi(\xv))^{1-y} \\
\logl & = ln(\LL) \\
& = y\ln{(\pi(\xv))} + (1-y)\ln{(1-\pi(\xv))}
\end{align}
