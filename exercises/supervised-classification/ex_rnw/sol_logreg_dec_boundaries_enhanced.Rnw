\begin{enumerate}[a)]
  ...
  \item We evaluate
  \begin{flalign*}
    \hat{y} = 1 &\Leftrightarrow \pix = \frac{1}{1 + \exp(-\thetab^T \xv)} \geq 
    \alpha \\
    &\Leftrightarrow\  1 + \exp(-\thetab^T \xv) \leq \frac{1}{\alpha} \\
    &\Leftrightarrow\ \exp(-\thetab^\top \xv) \leq \frac{1}{\alpha}  - 1 \\
    &\Leftrightarrow\ -\thetab^\top \xv \leq \log \left(\frac{1}{\alpha} - 1
    \right) \\
    &\Leftrightarrow\ \thx \geq -\log \left(\frac{1}{\alpha} - 1
  \right).
  \end{flalign*}
  $\thx = -\log \left(\frac{1}{\alpha} - 1 \right)$ is the equation of a linear 
  hyperplane comprised of all linear combinations $\thx$ that are equal to 
