\begin{enumerate}[a)]
  ...
  Now we want to compute for a new fruit the posterior probability 
  $\hat{\pi}_{yes}((\text{yellow}, \text{round}, \text{imported}))$.
  
  Note that we do not know the \emph{true} prior probability and the \emph{true} 
  conditional densities. Here -- since the target and the features are 
  categorical -- we can estimate them via the relative frequencies encountered 
  in the data:
  \begin{align*}
  ...
  \item Before, we only had categorical features and implicitly used a 
  categorical distribution, i.e., the simplest distribution over a $g$-way 
  event that is fully specified by the individual probabilities for each class 
  (which must obviously sum to 1), thus extending the Bernoulli distribution to 
  the multi-class case.
  This way, we could use the empirical frequencies as our distributional 
  parameters.
  ...
  In order to fully specify these normal distributions we also need to estimate 
