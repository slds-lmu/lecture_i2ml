...
<<echo=FALSE, message=FALSE, warning=FALSE, fig.height=2, fig.width=2>>=
p = p + geom_point(data.frame(x = c(0, 1), y = c(-1, -2)), mapping = aes(x, y), shape = "cross", col = "red") +
...
<<echo=FALSE, message=FALSE, warning=FALSE, fig.height=2, fig.width=2>>=
p = p + geom_point(data.frame(x = c(0, 1), y = c(0, -0.5)), mapping = aes(x, y), shape = "cross", col = "blue") +
  geom_abline(intercept = 0, slope = -0.5, col = "blue", linetype = 2) 
p
@
\end{itemize}

We see that yperplanes are defined via the points that lie directly on it and thus fulfill the hyperplane equation.
