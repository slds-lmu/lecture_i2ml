\begin{enumerate}[a)]
  ...
  \item \begin{enumerate}[i)]
    ...
    \textcolor{blue}{@Ludwig: die Datenlage ist tatsächlich etwas suboptimal, 
...
ggplot(data.frame(x = c(0, 8)), aes(x = x)) +
  ...
  stat_function(fun = dnorm, n = 100, args = list(mean = 4, sd = 2), aes(col = "3")) +
...
@
    ...
    \item Since LDA assumes isotropic Gaussians, i.e., constant variances across 
    all classes (also if this does not reflect the data siutation), all densities 
    would have the same shape and only differ in location. 
    
    \item As we have already noted, the assumption of equal class variances is 
    not justified here, but LDA is confined to isotropic distributions. 
  ...
  For $\xv_{\ast 2}$ the case is unambiguous and we obviously have 
  $\hat z_{\ast 2} = 2$.
  ...
  equal -- it is the one location where we would not be able to assign points 
  to either one of the classes, whereas for $\delta_1(\xv) > \delta_2(\xv)$ we 
  would assign class 1 and vice versa.
  ...
  \end{align*}
  The right hand side might look somewhat intimidating, but simply evaluates to 
