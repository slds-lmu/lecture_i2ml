...
\begin{enumerate}[a)]
  ...
  (\texttt{regr.rpart}) to the \texttt{bike\_sharing} task for
  \begin{itemize}
    \item \texttt{maxdepth} $\in \{2, 5, 10\}$ with \texttt{minsplit} $= 2$
    \item \texttt{minsplit} $\in \{2, 1000, 10000\}$ with \texttt{maxdepth} $= 20$
  \end{itemize}
  What do you observe?
