...
However, there is still some discussion about some properties of the algoritm.
...
\begin{enumerate}\bfseries
  \item[4)] Calculate the number of possible split points for splitting the root node.
\end{enumerate}
