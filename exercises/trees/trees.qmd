...
a) Compute the first split point the CART algorithm would find for each data set (with pen and paper or in `R`, resp. `Python`). Use mean squared error (MSE) to assess the empirical risk.
...
b) Derive the optimal constant predictor for a node $\Np$ when minimizing the empirical risk under $L2$ loss and explain why this is equivalent to minimizing variance impurity.
...
c) Explain why performing further splits can never result in a higher overall risk with $L2$ loss as a splitting criterion.
...
b) Using `mlr3`, fit a regression tree learner (`regr.rpart`) to the `bike_sharing` task (omitting the `date` feature) for
...
b) Using the randomized rule, compute the expected MCE in node $\Np$ that contains $n$ random training samples. What do you notice?
