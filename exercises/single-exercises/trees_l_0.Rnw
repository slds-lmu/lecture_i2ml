...
@
Example calculation of sum of squares for split at $x \leq 1$: $$SS_{total} = SS_{left} + SS_{right} = 
(1 - 1)^2 + ((1 - 5.625)^2 + (0.5 - 5.625)^2 + (10 - 5.625)^2 + (11 - 5.625)^2) = 95.6875 $$
...
split = function(x, y) {
  ...
  splits = lapply(x, function(i) {
    yleft = y[x <= i]
    yright = y[x > i]
    ss_total = SS(yleft) + SS(yright) # ... compute SS in both groups
    print(sprintf("Split at %.1f: %.2f SS", i, ss_total))
    ss_total
  })
