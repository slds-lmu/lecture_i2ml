...
\begin{align*}
Var(f(x)) &= \frac{1}{B^2} \left( \sum_{b=1}^B Var(f_b(x)) + \sum_{i \neq j}^B Cov(f_b(x), f_b(x)) \right ) \\
...
In the 1. line the rules for variance of a non independent sum of random variables is used. All other steps are trivial.
