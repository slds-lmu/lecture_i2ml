...
set.seed(123)
x_train <- seq(10, 15, length.out = 49)
...
ggplot2::ggplot(data_train, ggplot2::aes(x = x, y = y)) +
  ggplot2::geom_point() +
  ggplot2::geom_smooth(method = lm, formula = y ~ poly(x, 13), se = FALSE) + 
  ggplot2::geom_smooth(
...
\begin{enumerate}[a)]
  ...
  well, and end up with a degree-13 polynomial (blue, solid).
  ...
  (Hint: use \texttt{R} if you don't feel like computing a degree-13 polynomial 
  regression by hand.)
    <<echo=TRUE, message=FALSE>>=

set.seed(1)
x_test <- seq(10, 15, length.out = 29)
y_test <- 10 + 3 * sin(0.15 * pi * x_test) + rnorm(length(x_test), sd = 0.1)
data_test <- data.frame(x = c(x_test, 12), y = c(y_test, 18))
data_test
