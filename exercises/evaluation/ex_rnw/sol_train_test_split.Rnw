\begin{enumerate}[a)]
  ...
  ordered by feature value, which is obviously not a good idea and leads to a 
  pretty high test MSE (relatively speaking -- we will see in the next exercise 
  ...
  \begin{flalign*}
    \E_{\Pxy} ((\hat y - y)^2 ~|~ \xv) &= \E_{\Pxy} (\hat y^2 - 2\hat y y +
    y^2 ~|~ \xv) \\
    &= \hat y^2 - 2 \cdot \hat y \cdot \E_{\Pxy}(y ~|~ \xv) + 
    \E_{\Pxy}(y^2 ~|~ \xv) \\
    &= \hat y^2 - 2 \cdot \hat y \cdot \E_{\Pxy}(y ~|~ \xv) + 
    \E^2_{\Pxy}(y ~|~ \xv) +
    \var_{\Pxy}(y ~|~ \xv) \\ 
    &= \left(\hat y - \E_{\Pxy}(y ~|~ \xv) \right)^2 + \var_{\Pxy}(y ~|~ \xv).
  \end{flalign*}
  
  Now we incorporate our random sampling process and obtain:
  
  \begin{flalign*}
    \E_{\mathcal{S}, \Pxy} ((\hat y - y)^2 ~|~ \xv) &=
    \E_{\mathcal{S}} \left( \left(\hat y - \E_{\Pxy}(y ~|~ \xv) \right)^2 \right)
    + \var_{\Pxy}(y ~|~ \xv) \\
    &= \E_{\mathcal{S}} \left(\hat y^2 - 2 \cdot \hat y  \cdot 
    \E_{\Pxy}(y ~|~ \xv) +
    \E^2_{\Pxy}(y ~|~ \xv)\right) + \var_{\Pxy}(y ~|~ \xv) \\
    &= \E_{\mathcal{S}}(\hat y^2) - 2  \cdot \E_{\mathcal{S}}(\hat y)
     \cdot \E_{\Pxy}(y ~|~ \xv) + \E^2_{\Pxy}(y ~|~ \xv) + 
     \var_{\Pxy}(y ~|~ \xv) \\
    &= \E^2_{\mathcal{S}}(\hat y) + \var_{\mathcal{S}}(\hat y)-  
    2  \cdot \E_{\mathcal{S}}(\hat y)
     \cdot \E_{\Pxy}(y ~|~ \xv) + \E^2_{\Pxy}(y ~|~ \xv) + 
     \var_{\Pxy}(y ~|~ \xv) \\
  ...
  that $\E_{\Pxy}(y ~|~ \xv)$ is the best possible prediction we could make -- 
  the conditional expectation of our target under the true data-generating 
  process.
  Unfortunately, we must expect our prediction $\E_{\mathcal{S}}(\hat y)$ to be 
  different from $\E_{\Pxy}(y ~|~ \xv)$. 
  ...
  The second summand $\var_{\mathcal{S}}(\hat y)$ represents, unsurprisingly, 
  ...
  $$\E_{\mathcal{S}, \Pxy} ((\hat y - y)^2 ~|~ \xv) = 
  \left( \underbrace{\E_{\mathcal{S}}(\hat y) -  \E_{\Pxy}(y ~|~ \xv)}_{
  \text{bias}} \right)^2
  + \underbrace{\var_{\mathcal{S}}(\hat y)}_{\text{variance}} 
  + \underbrace{\var_{\Pxy}(y ~|~ \xv)}_{\text{irreducible error}}.$$
  
  Revisiting our practical experiments, we see again that 
  $\E_{\mathcal{S}}(\hat y)$ will be closer to the optimal prediction if we 
  have more data for training, while $\var_{\mathcal{S}}(\hat y)$ is large if 
