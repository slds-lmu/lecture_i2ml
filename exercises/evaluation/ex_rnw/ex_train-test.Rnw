...
\end{enumerate}
In addition to an unknown data-generating process $\Pxy$, supervised learning is often restricted to a data set $\D$ of fixed size $n$. Therefore, the true generalization error $\GE(\inducer, \ntrain, \rho)$ remains unknown. In this case, hold-out splitting is a simple  procedure that can be used to estimate the generalization error:
\begin{align}
