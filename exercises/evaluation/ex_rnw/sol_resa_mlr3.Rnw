...
@
  ...
  \item We see that generalization error estimates are pretty stable for the
  ...
  on the estimate.
  However, we see a glaring optimistic bias when we use a single 90\%-10\% 
  ...
  take a very long time, and the results from c) show that repeated CV with a 
  sufficient number of folds yields pretty stable estimates.
\end{enumerate}