...
set.seed(123)
x = runif(1000, 0, 50)
y = sin(1 / pi * x + pi) + rnorm(length(x), sd = 0.1)
...
<details>
<summary>**Solution**</summary>
- In this (stylized) example, the discretization of GS will prevent us from ever exploring promising values for $\lambda$:
```{r}
#| echo: false
#| fig.height: 4
#| fig.width: 6
...
p = ggplot(data.frame(x, evals, algorithm), aes(x = x, y = evals, col = algorithm)) +
  geom_line() +
  geom_point() +
  labs(x = "budget", y = expression(paste("evaluations of", lambda[1]))) +
  theme_minimal()
print(p)
```
...
You receive the following `mlr3` output from tuning $k$-NN.
...
terminator_evals <- trm("evals", n_evals = 20)
terminator_runtime <- trm("run_time", secs = 10)
terminator <- trm(
	"combo", 
