...
b) Consider the following objective function $c(\lambda)$ with a single hyperparameter $\lambda \in \R$. The objective is to be minimized. Explain whether RS or GS will be more suited to find the optimal value $\lambda^\ast$, given
...
c) Consider now a bivariate objective function $c(\lambda_1, \lambda_2)$ with $\lambda_1, \lambda_2 \in \R$. The objective is to be minimized. Suppose that $\lambda_2$ is vastly more important for the objective than $\lambda_1$.
