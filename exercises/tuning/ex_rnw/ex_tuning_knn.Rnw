...
\begin{enumerate}[a)]
  ...
  Set aside 200 test observations.
  ...
  Where are the limits of such analysis?
  
  \item After analyzing the tuning results, you notice that changes in $k$ are 
  much more influential for smaller neighborhoods (2 vs 7 neighbors is a lot 
  more different than 100 vs 105).
  Re-run the HPO procedure with a log-transformation for $k$.
  
  \item With the hyperparameter configuration found via HPO, fit the nmodel on 
