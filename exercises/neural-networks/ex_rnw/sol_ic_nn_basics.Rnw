...
In the context of an regression/classification problem the type of activation function used in the output layer i.a. depends on the assumed data distribution of the target variable y and thereby often corresponds to the link-function used within regression approaches.
\end{enumerate}
