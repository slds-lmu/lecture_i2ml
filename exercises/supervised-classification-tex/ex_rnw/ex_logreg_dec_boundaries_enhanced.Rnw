...
$\pixt \geq \alpha$ for some $\alpha \in (0, 1)$.

\begin{enumerate}[a)]
  \item Show that the decision boundary of the logistic classifier is a 
  (linear) hyperplane. \\
  Hint: derive the value of $\thx$  (depending on 
  ...
  with two input features for different values $\thetab = (\theta_1, 
  \theta_2)$ 
  (plots 1-3) as well as $\alpha$ (plot 4). 
  What can you deduce for the values of $\theta_1$, $\theta_2$ and 
