Binary logistic regression is a special case of multiclass logistic, or \emph{softmax}, regression.
The softmax function is the multiclass analogue to the logistic function, transforming scores $\thx$ to values $\in [0, 1]$ that sum to one: 
$$\pikxt = \frac{\exp(\thetab_k^\top \xv)}{\sum_{j = 1}^g \exp(\thetab_j^\top \xv)}, ~~ k \in \gset$$
