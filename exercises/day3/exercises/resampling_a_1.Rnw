We fit a k-NN model to predict the pixel classes in the Satellite dataset. The performance is evaluated with the kappa coefficient, which measures the agreement between two scorers: The higher the kappa, the better the agreement (max 1) between predicted and actual classes. \\
