...
\begin{enumerate}
	...
	%
	is the optimal constant model for the loss function above.
	...
	\emph{Hint:} Use the law of total variance, which states that $ \var(X) = \E_X\left[\var(Y~|~X)\right] + \var(  \E_{Y|X} \left[ Y ~|~ X \right] ),$ where the conditional variance is defined as $\var(Y~|~X) = \E_X \left[  \big(Y - \E_{Y|X}(Y ~|~X)\Big)^2 ~|~X \right].$
