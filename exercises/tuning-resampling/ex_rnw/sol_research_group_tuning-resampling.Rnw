...
\item[1)] \begin{itemize}
  \item Hyperparameter tuning means finding the hyperparameter vector $\bm{\lambda}$ s.t. the model trained by learner $\ind$ with hyperparameter vector $\bm{\lambda}$ on $\Dtrain$ and evaluated on $\Dtest$, results in the lowest estimated generalization error.
  \item Tuning is a bi-level optimization problem because the \textbf{empirical risk minimization problem} on the level of the learner $\ind$ depends on the hyperparameters provided to $\ind$, and is therefore nested within the \textbf{hyperparameter optimization problem}.
\end{itemize}
