...
@  
  ...
  Note that the $L2$-induced multiplicative factor of 2 in our gradient 
  ...
  So, what actually happens here: we update each component of our current 
  ...
  empirical risk, and it is reasonable to assume that it is rather important for 
  the model.
  
  Therefore, gradient updates focus on  influential components, and we 
  proceed more strongly along the important dimensions.
  ...
  from an entirely different model class) might be advantageous:
  \begin{itemize}
