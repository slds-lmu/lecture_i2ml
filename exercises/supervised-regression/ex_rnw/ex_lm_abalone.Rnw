...
\begin{itemize}
  \item[a)] Plot \texttt{LongestShell} and \texttt{WholeWeight} on the $x$- and 
  $y$-axis, respectively, and color points with \texttt{Rings}.
\end{itemize}

Using \texttt{mlr3}:

\begin{itemize}
  \item[b)] Fit a linear model (for this you will need the \texttt{mlr3learners} 
  extension package).
...
Recall that you can find the official \texttt{mlr3} manual at 
