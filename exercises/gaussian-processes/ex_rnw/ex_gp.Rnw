...
\item Implement the GP with squared exponential kernel, zero mean function and $\ls = 1$ from scratch for $n=2$ observations $(\bm{y},\bm{x})$ and $\bm{\Psi} = \bm{I}$. 
Do this as efficiently as possible by explicitly calculating all expensive computations by hand. Do the same for the posterior predicitve distribution of $y_*$. Test your implementation using simulated data.
\end{enumerate}
