...
Son suggests to use L2 loss for their linear regression model, and notes this would require no numerical optimization to find the parametrization with lowest empirical risk.
...
The research group decides to train the polynomial regression model using L2 loss. Again, Son is worried about the optimization component of learning. He is not entirely sure how to find the parametrization with lowest empirical risk.
