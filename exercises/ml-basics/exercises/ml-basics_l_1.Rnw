\begin{enumerate}[a)]
  ...
  \item $\Hspace = \{\pi: \Xspace \mapsto \R | \pixt = s(\thetab^T \xv) \}$,
  ...
  log-likelihood as our loss function: $\Lpixyit = -\yi \log(\pixit) +
  (1 - \yi)(\log(1 - \pixit))$ (the so-called \emph{Bernoulli loss}). The
  empirical risk is then the sum of point-wise losses: $\risket = -\sumin \yi
  \log(\pixit) + (1 - \yi)(\log(1 - \pixit))$.
  ...
  Therefore, we set the first derivative of $\risket$ wrt $\theta$ to 0 and
  solve for $\theta$. However -- unlike linear regression -- this has no
