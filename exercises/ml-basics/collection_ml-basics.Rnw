...
\LARGE
\textbf{Collection of exercises -- ML Basics}
\normalsize
...
\hrule
\Large
\textbf{Lecture exercises}
\normalsize
\dlz

\aufgabe{
<<child="ex_rnw/ex_types_1.Rnw">>=
@
}

\dlz
\aufgabe{
<<child="ex_rnw/ex_lin_reg_1.Rnw">>=
@
}

\dlz
\aufgabe{
<<child="ex_rnw/ex_ml_basics_1.Rnw">>=
@
}
...
\hrule
\Large
\textbf{Tasks from past exams}
\normalsize
\dlz

\textbf{WS2020/21, retry exam}
...
\begin{enumerate}
  \item Now we want to train a cubic polynomial, i.e., a polynomial regression 
...
\hrule
\Large
\textbf{Ideas \& exercises from other sources}
\normalsize