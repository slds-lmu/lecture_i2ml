...
\medbreak
The figure below plots three different classification models for the same data set. An observation with numerical features $\xv = (x_1, x_2)^T$ can either belong to class 1 or class 2. The true class is indicated by the color of the corresponding point.
The class a model predicts is indicated by the background color. For example, the single observation with $\xv = (5, 0)^T$ belongs to class 1, which is correctly predicted by model A and model C, but incorrectly predicted by model B.
\bigbreak
...
points <- data.frame(x1 = runif(299, 0, 5),
                     x2 = runif(299, 0, 5),
                     Class = rep(1, 299))
points[,3] <- as.factor(mapply(function(x, y) if (y > -(x-1.6)*(x-1.2)*(x-4.3)) {sample.int(2, size = 1, prob = c(0.02, 0.98))} else {1}, points[,1], points[,2]))
points[300,] <- c(4.95, 0.05, as.factor(1))
...
To predict the class of a new observation with features $\xv = (x_1, x_2)^T$, take the 5 closest points in the data set (by their euclidean distance) and select the class which is most common among these 5 points.
\bigbreak
\item \textbf{CART} \\
...
\cline{3-4}
& & \multicolumn{2}{ c| }{Response} \\ \cline{3-4}
& & 1 & 2 \\ \cline{1-4}
\multicolumn{1}{ |c  }{\multirow{2}{*}{Truth} } &
\multicolumn{1}{ |c| }{1} & 57 & 52 &     \\ \cline{2-4}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{2} & 37 & 154 &     \\ \cline{1-4}
\end{tabular} &
\begin{tabular}{cc|c|c|l}
\cline{3-4}
& & \multicolumn{2}{ c| }{Response} \\ \cline{3-4}
& & 1 & 2 \\ \cline{1-4}
\multicolumn{1}{ |c  }{\multirow{2}{*}{Truth} } &
\multicolumn{1}{ |c| }{1} & 100 & 9 &     \\ \cline{2-4}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{2} & 6 & 185 &     \\ \cline{1-4}
\end{tabular} &
\begin{tabular}{cc|c|c|l}
\cline{3-4}
& & \multicolumn{2}{ c| }{Response} \\ \cline{3-4}
& & 1 & 2 \\ \cline{1-4}
\multicolumn{1}{ |c  }{\multirow{2}{*}{Truth} } &
\multicolumn{1}{ |c| }{1} & 98 & 11 &     \\ \cline{2-4}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{2} & 15 & 176 &     \\ \cline{1-4}
\end{tabular} \\
\end{tabular}
\end{center}
