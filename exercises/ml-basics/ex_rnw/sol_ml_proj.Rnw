\begin{enumerate}[a)]
  ...
  We can thus simply use the \textbf{negative log-likelihood} as our loss 
  ...
  \item In order to find the optimal $\thetab$, we need to solve the following 
  minimization problem: 
  
  \begin{flalign*}
    \min_{\thetab \in \Theta} \risket &= \min_{\thetab \in \Theta} \left( \sumin 
    \left(\yi - \thetab^T \xi \right)^2 \right) \\
    &= \min_{\thetab \in \Theta} \| \ydat - \Xmat \thetab \|_2^2
  \end{flalign*}  
