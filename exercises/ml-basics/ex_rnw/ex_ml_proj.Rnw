...
\begin{enumerate}[a)]
  ...
  \item Assume now that you have data on vehicles’ age, mileage, and price. 
  Explicitly define the feature space $\Xspace$ and target space $\Yspace$ and 
  state the formal notation for an exemplary observation.
  ...
  The linear hypothesis models the target as a linear function of the covariates 
  ...
  likelihood estimation (of course, in the special case of the LM, we also have 
  a direct analytical solution given by the least-squares estimator). 
  ...
  \frac{1}{2 \sigma^2} \left( \yi - \thetat \xi  \right)^2  \right) \\
  ...
  and write down the resulting loss function.
  \item Now you need to optimize this risk function to find the best parameters 
  and hence the best model via empirical risk minimization. 
  List the necessary steps to solve the optimization problem. 
