...
(@) Demonstrate that this hypothesis space is simply a parameterized family 
  of curves by plotting curves for 3 different models belonging to 
  the considered model class.
  
(@) State the empirical risk w.r.t. $\boldsymbol{\theta}$ for a member of the hypothesis 
  space. Use $L2$ loss and be as explicit as possible.

(@) We can minimize this risk using gradient descent. In order to make 
