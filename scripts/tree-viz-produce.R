...
mce <- rep(0, length(possible_splits))
right_plot <- base + theme_bw() + theme(legend.position="top")
for (s in 1:length(possible_splits)) {
  rgt  <- names(which.max(table(iris2$prediction[iris2$Sepal.Width >= possible_splits[s]])))
  lft <- names(which.max(table(iris2$prediction[iris2$Sepal.Width < possible_splits[s]])))
  newprediction <- ifelse(iris2$Sepal.Width >= possible_splits[s], rgt, lft)
  mce[s] <- (sum(newprediction != iris2$prediction)) / nrow(iris2)
  right_plot <- right_plot + annotate("rect", xmin = -Inf, xmax = Inf, col = "black",
                                      ymin = possible_splits[s], ymax = possible_splits[s]) +
    annotate("text", x = 8.25, y = possible_splits[s] + 0.07, label = paste0("MCE = ", round(mce[s], 2))) +
    xlim(c(4, 8.5)) 
}
