...
\begin{itemize}
  ...
  \begin{itemize}
    \item manhattan distance:
      $$d(\xi,\xb^{(j)})= ||\xi - \xb^{(j)}||_1 = \sum_{k=1}^p|x^{(i)}_k-x^{(j)}_k|$$
    \item euclidean distance:
      $$d(\xi,\xb^{(j)})= ||\xi - \xb^{(j)}||_2 = \sqrt{\sum_{k=1}^p(x^{(i)}_k-x^{(j)}_k)^2}$$
  \end{itemize}
...
\begin{itemize}
  ...
  \item For an arbitrary cluster $j$ define its \textit{index space} by
  \[C_j\coloneqq\{i\subset\{1,\ldots,k\}|\xi\text{cbelongs to cluster }j\}\]
...
\[d_{\text{average}}(C_r,C_s) = \frac{1}{n_r n_s} \sum_{i \in C_r} \sum_{i \in C_s} d(\xi,\xb^{(j)})\]
\end{vbframe}
