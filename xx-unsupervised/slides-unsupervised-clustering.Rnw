...
\begin{vbframe}{Hierarchical Clustering}
Let $\Xspace$ be input space of $n$ observations with $p$ features (dimensions), where
...
We now "forget" the real groups specified by the \code{Species} variable and try to find clusters based on the leaf measurements.
