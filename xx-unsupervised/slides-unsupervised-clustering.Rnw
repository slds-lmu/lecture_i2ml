...
\begin{itemize}
  \item \small manhattan: sum up the absolute distances in each dimension.
      In R: \code{dist(data, method $=$ "manhattan")}
  \item \small euclidean: remember Pythagoras theorem from school?
      In R: \code{dist(data, method $=$ "euclidean")}
  \item \small gower: can be used for mixed variables (categorical and numeric).
      In R: \code{gower\_dist()} from the \pkg{gower} package
  \item \small see \code{?dist} for other distances.
\end{itemize}
\end{vbframe}
...
<<echo = FALSE, fig.height=4, out.width="0.75\\textwidth">>=
...
<<fig.width = 7, fig.height = 5, fig.align="center", echo=FALSE, message=FALSE, results="asis", out.width="0.9\\textwidth">>=
set.seed(1234)
pts = iris[, 3:4]
...
\begin{itemize}
  ...
  In R, the maximum number of iterations is specified by \code{iter.max}. \\
  ...
  \item $K$-means can be restarted multiple times using \code{nstart}.
...
<<echo=FALSE, fig.width=8, fig.height=4, out.width="0.9\\textwidth">>=
set.seed(0)
x = rbind(
  scale(matrix(rnorm(2*20, sd = 0.4), ncol=2), center=c(1,1), scale = F),
  scale(matrix(rnorm(2*30, sd = 0.4), ncol=2), center=-c(1,1), scale = F)
...
\begin{itemize}
  \item Minimizing the \textit{within-cluster variation} exactly is not feasible and can be approximated by the $K$-means algorithm.
  ...
  \textit{within-cluster variation} for several $K$ and look for an \textit{"elbow"} which is a good guess for $K$.
\end{itemize}
\end{vbframe}
