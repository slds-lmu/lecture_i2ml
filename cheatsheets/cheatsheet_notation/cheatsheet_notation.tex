...
\textcolor{orange}{$\bm{\Lambda} = \bm{\Lambda_1} \times \bm{\Lambda_2} \times ... \times \bm{\Lambda_}{\ll} \subset \R^{\ll}$, where $\bm{\Lambda_j} = (a_j, b_j), \quad a_j, b_j \in \R,$ \\$j = 1, 2, ... , \ll$ : \textbf{hyperparameter space} }\\
...
For $\Yspace = \setmp$: $\textcolor{orange}{\Lpixy = \log(1 + (\frac{\pix}{1-\pix})^{-y})}$ \\
