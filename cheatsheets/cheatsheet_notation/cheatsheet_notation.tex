...
\in \setzo^n$ : \textcolor{orange}{one-hot encoding for $i$-th observation, belonging to class k  (egtl. ja ne matrix?)} \\
...
\textcolor{orange}{
$\fkx: \Xspace \rightarrow \R^p$: \textbf{discriminant functions} for class $k$ (in a binary case we might abbreviate to $\fx = f_{1}(\xv) - f_{2}(\xv)$)}\\
 
\textcolor{orange}{
$\hx: \R^g \rightarrow \Yspace$ : \textbf{hard label function} for classification that maps class scores / posterior probabilities to discrete classes. Typically created by $h(\xv) = \argmax_{k \in \gset} \fkx$ or $h(\xv) = \argmax_{k \in \gset} \pikx$}  \\
...
\begin{myblock}{Loss and Risk}
 ...
 Quantifies "quality" $\Lxy$ of prediction $\fx$ (or $\Lpixy$ of prediction $\pikx$) for single $\xv$. \\
...
\begin{itemize}    
  ...
  \bar{y}$
  \item Optimal model for unrestricted $\Hspace$: $\fxh = \meanin\E (\yi | \xi)$
...
\setlength{\itemindent}{+.3in}
  ...
  \item Optimal constant model: $\fxh = \text{med}(\yi)$
   \item Optimal model for unrestricted $\Hspace$: $\fxh = \meanin \text{med}(\yi | \xi)$
