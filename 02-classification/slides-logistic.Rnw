...
For now we focus on the binary case $y \in \{0, 1\}$.

\lz 

A naive approach would be to model
\[
\pix = \post = \thetab^\top \xb.
\]
Obviously this could result in predicted probabilities $\pix \not\in [0,1]$.

\framebreak

To avoid this, logistic regression \enquote{squashes} the estimated linear scores $\thetab^\top \xb$ to $[0,1]$ through the \textbf{logistic function} $s$:
\[
\pix = \post = \frac{\exp\left(\thetab^\top \xb\right)}{1+\exp\left(\thetab^\top \xb\right)} = \frac{1}{1+\exp\left(-\thetab^\top \xb\right)} = s\left(\thetab^\top \xb\right)
\]
\lz

Note that we will again usually suppress the intercept in notation, i.e., $\thetab^\top \xb \equiv \theta_0 + \thetab^\top \xb$.
...
$\post = s(\thetab^\top \xb)  = \frac{\exp(\thetab^\top \xb)}{1+\exp(\thetab^\top \xb)} $
\lz
...
Let us assume $\theta_1 = 1$ for simplicity, so that
...
In order to find the \enquote{optimal} model represented by $\thetab$, we need to define a \emph{loss function}.\lz
...
\text{\enquote{accuracy}}^{(i)} &:=
  \begin{cases} \pi(\xi, \thetab) & \text{if } \yi = 1 \\
              1-\pi(\xi, \thetab) & \text{if } \yi = 0 \end{cases} \\
  & = \pi(\xi, \thetab)^{\yi} (1-\pi(\xi, \thetab))^{1-\yi}
...
target vector given the model:}
  \text{\enquote{global accuracy}} &:= \prod^n_{i=1} \pi(\xi, \thetab)^{\yi} (1-\pi(\xi, \thetab))^{1-\yi}
\intertext{We want a \emph{loss} function, so we actually need the inverse of that:}
  \text{\enquote{global inacccuracy}} &:= \frac{1}{\prod^n_{i=1} \pi(\xi, \thetab)^{\yi} (1-\pi(\xi, \thetab))^{1-\yi}}
...
\intertext{so we turn the product into a sum by taking its log -- the same parameters minimize this, which is all we care about, and we end up with the \textbf{logistic} or \textbf{cross entropy loss function}:}
 \Lxy &= - y \log[\pix] - (1-y) \log [1 - \pix] \\
      & = - y \thetab^\top \xb + \log[1 + \exp(\thetab^\top \xb)]
\end{align*}

\framebreak

Remember that $\log[\pix] = - \log[1 + \exp(-\thetab^\top \xb)]$.
\lz

For $y=0$ and $y=1$ and $\fx = \thetab^\top \xb$, this yields:
\begin{eqnarray*}
y=0 &:& \log[1 + \exp(\fx] \\
y=1 &:& \log[1 + \exp(-\fx)]
\end{eqnarray*}
...
$$
\pi(\xb, \thetab) = \P(y = 1 | \xb, \thetab) = \frac{\exp(\thetab^\top \xb)}{1+\exp(\thetab^\top \xb)} \ge 0.5,
$$

which is equivalent to
$$
\thetab^\top \xb \ge 0 \implies y = 1.
$$
...
$$
\pikx = \P(y = k ~|~ \xb) = \frac{\exp(\thetab_k^\top \xb)}{\sum_{j=1}^g\exp(\thetab_j^\top \xb)}.
$$
...
By comparing the posterior probabilities of two categories $k$ and $l$ we end up in a linear function (in $\xb$),

$$
\log\frac{\pi_k(\xb)}{\pi_l(\xb)} = (\thetab_k-\thetab_l)^\top \xb.
$$
...
\textbf{Remark:}
\begin{itemize}
\item $\thetab_j$ are vectors here.
\item Well-definedness: $\pi_k(\xb) \in [0, 1]$ and $\sum_k \pi_k(\xb) = 1$
...
we define a binary score model $f_k(\xb)$ with parameter vector $\thetab_k$.
We then combine these models through the softmax function

$$
\postk = \pikx = s_k(f_1(\xb), \ldots f_g(\xb))
$$
...
\item For linear $\fxt = \thetab^\top \xb$, this is also called \emph{softmax regression}. (Note that $x$ can include derived features like polynomials or interactions as well.)
\item One set of parameters is  \enquote{redundant}: If we subtract any fixed vector
  from all $\thetab_k$, the predictions do not change. The model is \enquote{overparameterized}, the minimizer of $\risket$ is not unique. Hence, we set $\thetab_g = (0, \dots, 0)$ and only optimize the other $\thetab_k$, $k=1,\dots,g-1$.\\ (Compare: logistic regression for binary classification also has only \emph{one} parameter vector for discriminating between two classes...).
...
\textbf{Representation:} Design matrix $\Xmat$, coefficients $\thetab$.\\
