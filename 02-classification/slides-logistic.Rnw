...
Logistic regression squashes the estimated linear scores $\theta^T x$ to $[0,1]$ through a sigmoid function $s$:
\[
...
<<logistic-function, echo=FALSE, out.height="0.65\\paperheight", fig.height = 6>>=
n = 100
...
We assume an intercept only model for simplicity: $\post =  \frac{\exp(\theta_0)}{1+\exp(\theta_0)} $

<<logistic-function-par, echo=FALSE, out.height="0.65\\paperheight">>=
n = 100
...
scale_x_continuous('t')  + scale_y_continuous('s(t)') + scale_color_discrete(expression(theta*"0"))
@

\framebreak
Assuming a single feature and no intercept: $\post =  \frac{\exp(\theta_1 x_1)}{1+\exp(\theta_1 x_1)}$:
Parameter $\theta_1$ controls slope and direction of the logistic curve.

<<logistic-function-par2, echo=FALSE, out.height="0.65\\paperheight">>=
n = 100
...
scale_x_continuous('t')  + scale_y_continuous('s(t)') + scale_color_discrete(expression(theta*"1"))
@
