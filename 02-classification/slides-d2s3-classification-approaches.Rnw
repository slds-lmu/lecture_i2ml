...
classb = data.frame(mvrnorm(n = 1000, mu = c(10,10), Sigma = matrix(c(4, -3, -3, 4), ncol = 2, byrow = TRUE)))
...
\begin{itemize}
  \item In the late nienties, Naive Bayes became popular spam-filter program for email
