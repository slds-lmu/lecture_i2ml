...
\begin{vbframe}{Linear classifier}
  ...
  if we opt to manually construct \emph{derived features} like higher order interactions, polynomials or other
...
\begin{itemize}
\item \emph{Generative} classification approaches assume a data generating process in which the distribution of the features $x$ is different between the classes of the output $y$ and try to learn these conditional distributions:\\ \enquote{Which $y$ tends to have $x$ like these?}
\lz
...
\begin{vbframe}{Generative approach}
  ...
  and models $\pdfxyk$, usually by making some assumptions about the structure of these distributions,
  to allow the computation of $\pikx$. 
  
  The discriminant functions are simply $\pikx$ or $\lpdfxyk + \lpik$.\\
  
  The prior class probabilities $\pik$ are easy to estimate from the training data.
...
Note: LDA and QDA have 'discriminant' in their name, but are generative models! (\dots sorry.)
\end{vbframe}
